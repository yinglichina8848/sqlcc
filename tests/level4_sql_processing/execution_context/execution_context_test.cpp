...
TEST_F(ExecutionContextTest, BasicConstructor) {
    ...
    EXPECT_EQ(context.get_execution_plan(), "未优化");
...
TEST_F(ExecutionContextTest, ManagerConstructor) {
    ...
    // 使用不同的目录名避免冲突
    std::string unique_path = "./test_db_mgr_" + std::to_string(std::chrono::system_clock::now().time_since_epoch().count());
    auto db_mgr = std::make_shared<DatabaseManager>(unique_path);
