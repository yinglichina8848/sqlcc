...
void assertParseSuccess(const std::string &sql,
  ...
  try {
    ...
    if (!statements.empty()) {
      std::cout << "✅ " << description << " - parsed successfully ("
                << statements.size() << " statements)" << std::endl;
    } else {
      std::cout << "❌ " << description << " - no statements parsed"
                << std::endl;
      assert(false);
    }
  } catch (const std::exception &e) {
    std::cout << "❌ " << description << " - parse error: " << e.what()
              << std::endl;
    assert(false);
  }
}

void assertParseFailure(const std::string &sql,
                        const std::string &description) {
  try {
    ParserNew parser(sql);
    auto statements = parser.parse();
    std::cout << "❌ " << description
              << " - expected parse failure but succeeded" << std::endl;
    assert(false);
  } catch (const std::exception &e) {
    std::cout << "✅ " << description
              << " - correctly failed to parse: " << e.what() << std::endl;
  }
}
...
void testDDLStatements() {
  ...
  assertParseSuccess("CREATE TABLE users ("
  ...
  // CREATE INDEX
  assertParseSuccess("CREATE INDEX idx_username ON users (username);",
                     "CREATE INDEX");
...
void testDMLStatements() {
  ...
  // SELECT with WHERE
  assertParseSuccess("SELECT id, username, email FROM users WHERE age > 18 AND "
                     "status = 'active';",
                     "SELECT with WHERE");

  // SELECT with JOIN
  assertParseSuccess("SELECT u.name, p.title FROM users u INNER JOIN posts p "
                     "ON u.id = p.user_id;",
                     "SELECT with JOIN");

  // INSERT
  assertParseSuccess("INSERT INTO users (username, email, age) VALUES ('john', "
                     "'john@example.com', 25);",
                     "INSERT statement");
...
void testDCLStatements() {
  ...
  // GRANT
  assertParseSuccess("GRANT SELECT, INSERT ON TABLE users TO john;",
                     "GRANT privileges");

  // REVOKE
  assertParseSuccess("REVOKE INSERT ON TABLE users FROM john;",
                     "REVOKE privileges");

  // CREATE USER
  assertParseSuccess("CREATE USER admin IDENTIFIED BY 'password123';",
                     "CREATE USER");
...
void testComplexSQL() {
  ...
  assertParseSuccess("CREATE TABLE orders ("
  ...
  assertParseSuccess("SELECT u.username, COUNT(o.id) as order_count "
                     "FROM users u "
  ...
  // Multiple statements
  assertParseSuccess("CREATE DATABASE shop; "
                     "USE shop; "
...
void testComments() {
  ...
  // Mixed comments
  assertParseSuccess("/* Start */ SELECT * /* middle */ FROM users; -- End",
                     "Mixed comments");
...
void testLiteralsAndIdentifiers() {
  ...
  // String literals
  assertParseSuccess("SELECT * FROM users WHERE name = 'John';",
                     "String literals");

  // Numeric literals
  assertParseSuccess("SELECT * FROM products WHERE price > 99.99;",
                     "Numeric literals");

  // Scientific notation
  assertParseSuccess("SELECT * FROM data WHERE value > 1.23e10;",
                     "Scientific notation");

  // Identifiers with underscores
  assertParseSuccess("SELECT user_name, user_email FROM user_table;",
...
void testErrorHandling() {
  ...
  // Unterminated string
  assertParseFailure("SELECT * FROM users WHERE name = 'unterminated;",
                     "Unterminated string");
...
void testPerformanceSmoke() {
  ...
  std::cout << "✅ Performance smoke test: 100 complex parses in "
            << duration.count() << "ms" << std::endl;
  std::cout << "   Average: " << (duration.count() / 100.0) << "ms per parse"
            << std::endl;
}
...
int main() {
  ...
  try {
    ...
    std::cout << std::endl;
    std::cout << "✅ DFA lexer successfully integrated with SQL parser"
              << std::endl;
    std::cout << "✅ All SQL statement types parse correctly" << std::endl;
    std::cout << "✅ Token system migration completed successfully"
              << std::endl;
