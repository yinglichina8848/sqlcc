...
private:
  void tokenize() {
    ...
    while (pos < input_.length()) {
      ...
      if (std::isdigit(c)) {
        size_t start = pos;
        while (pos < input_.length() && std::isdigit(input_[pos]))
          pos++;
        ...
        TokenType numberType = (number.find('.') != std::string::npos)
                                   ? TokenType::FLOAT_LITERAL
                                   : TokenType::INTEGER_LITERAL;
        tokens_.emplace_back(numberType, number, line_, column_);
        column_ += pos - start;
        continue;
      }
      ...
      if (c == '\'') {
        ...
        tokens_.emplace_back(TokenType::STRING_LITERAL, str, line_, column_);
        column_ += pos - start;
        continue;
      }
      ...
      tokens_.emplace_back(TokenType::UNKNOWN, std::string(1, c), line_,
                           column_);
      column_++;
      pos++;
    }
...
public:
  ...
  const std::vector<std::unique_ptr<MockASTNode>> &getChildren() const {
    return children_;
  }
...
private:
  std::string astNodeTypeToString() const {
    ...
    case ASTNodeType::INSERT_STATEMENT:
      return "INSERT";
    case ASTNodeType::UPDATE_STATEMENT:
      return "UPDATE";
    case ASTNodeType::DELETE_STATEMENT:
      return "DELETE";
    case ASTNodeType::CREATE_STATEMENT:
      return "CREATE";
    case ASTNodeType::DROP_STATEMENT:
      return "DROP";
    case ASTNodeType::ALTER_STATEMENT:
      return "ALTER";
    case ASTNodeType::EXPRESSION:
      return "EXPR";
    case ASTNodeType::COLUMN_DEFINITION:
      return "COLUMN";
    case ASTNodeType::TABLE_REFERENCE:
      return "TABLE";
    default:
      return "UNKNOWN";
    }
...
class MockParserNew {
public:
  explicit MockParserNew(const std::string &input)
      : input_(input), lexer_(input) {}
...
private:
  std::unique_ptr<MockASTNode>
  parseStatement(const std::vector<MockToken> &tokens, size_t &pos) {
    if (pos >= tokens.size())
      return nullptr;
  ...
  std::unique_ptr<MockASTNode>
  parseSelectStatement(const std::vector<MockToken> &tokens, size_t &pos) {
    auto node =
        std::make_unique<MockASTNode>(ASTNodeType::SELECT_STATEMENT, "SELECT");

    // Parse select list
    if (pos < tokens.size() && tokens[pos].lexeme == "*") {
      node->addChild(
          std::make_unique<MockASTNode>(ASTNodeType::EXPRESSION, "*"));
      pos++;
    } else {
      // Parse column list
    ...
    if (pos < tokens.size() && tokens[pos].type == TokenType::FROM) {
      pos++;
      if (pos < tokens.size()) {
        node->addChild(std::make_unique<MockASTNode>(
            ASTNodeType::TABLE_REFERENCE, tokens[pos].lexeme));
        pos++;
      }
    }
    ...
    if (pos < tokens.size() && tokens[pos].type == TokenType::WHERE) {
      pos++;
      auto where =
          std::make_unique<MockASTNode>(ASTNodeType::EXPRESSION, "WHERE");
      if (pos < tokens.size()) {
        where->addChild(std::make_unique<MockASTNode>(ASTNodeType::EXPRESSION,
                                                      tokens[pos].lexeme));
        pos++;
      }
  ...
  std::unique_ptr<MockASTNode>
  parseInsertStatement(const std::vector<MockToken> &tokens, size_t &pos) {
    auto node =
        std::make_unique<MockASTNode>(ASTNodeType::INSERT_STATEMENT, "INSERT");
    ...
    // Table name
    if (pos < tokens.size()) {
      node->addChild(std::make_unique<MockASTNode>(ASTNodeType::TABLE_REFERENCE,
                                                   tokens[pos].lexeme));
      pos++;
    }
  ...
  std::unique_ptr<MockASTNode>
  parseUpdateStatement(const std::vector<MockToken> &tokens, size_t &pos) {
    auto node =
        std::make_unique<MockASTNode>(ASTNodeType::UPDATE_STATEMENT, "UPDATE");
    ...
    if (pos < tokens.size()) {
      node->addChild(
          std::make_unique<MockASTNode>(ASTNodeType::EXPRESSION, "SET_CLAUSE"));
      pos++;
    }
  ...
  std::unique_ptr<MockASTNode>
  parseDeleteStatement(const std::vector<MockToken> &tokens, size_t &pos) {
    auto node =
        std::make_unique<MockASTNode>(ASTNodeType::DELETE_STATEMENT, "DELETE");
  ...
  std::unique_ptr<MockASTNode>
  parseCreateStatement(const std::vector<MockToken> &tokens, size_t &pos) {
    auto node =
        std::make_unique<MockASTNode>(ASTNodeType::CREATE_STATEMENT, "CREATE");

    // TABLE or DATABASE keyword
    if (pos < tokens.size()) {
      std::string objectType = tokens[pos].lexeme;
      node->addChild(
          std::make_unique<MockASTNode>(ASTNodeType::EXPRESSION, objectType));
      pos++;
  ...
  std::unique_ptr<MockASTNode>
  parseDropStatement(const std::vector<MockToken> &tokens, size_t &pos) {
    auto node =
        std::make_unique<MockASTNode>(ASTNodeType::DROP_STATEMENT, "DROP");
  ...
  std::unique_ptr<MockASTNode>
  parseColumnList(const std::vector<MockToken> &tokens, size_t &pos) {
    auto list =
        std::make_unique<MockASTNode>(ASTNodeType::EXPRESSION, "COLUMN_LIST");
  ...
  std::unique_ptr<MockASTNode>
  parseValueList(const std::vector<MockToken> &tokens, size_t &pos) {
    auto list =
        std::make_unique<MockASTNode>(ASTNodeType::EXPRESSION, "VALUE_LIST");

    while (pos < tokens.size() && tokens[pos].type != TokenType::RIGHT_PAREN) {
      if (tokens[pos].type == TokenType::INTEGER_LITERAL ||
          tokens[pos].type == TokenType::FLOAT_LITERAL ||
          tokens[pos].type == TokenType::STRING_LITERAL) {
...
// Test cases for ParserNew integration
TEST(ParserNewIntegrationTest, BasicSelect) {
  MockParserNew parser("SELECT * FROM users;");
  auto statements = parser.parse();

  EXPECT_EQ(statements.size(), 1);
  if (statements.size() > 0) {
    EXPECT_EQ(statements[0]->getType(), ASTNodeType::SELECT_STATEMENT);
  }
}

TEST(ParserNewIntegrationTest, SelectWithWhere) {
  MockParserNew parser("SELECT name FROM users WHERE id = 1;");
  auto statements = parser.parse();
...
TEST(ParserNewIntegrationTest, InsertStatement) {
  MockParserNew parser("INSERT INTO users (name, age) VALUES ('John', 25);");
  auto statements = parser.parse();

  EXPECT_EQ(statements.size(), 1);
  if (statements.size() > 0) {
    EXPECT_EQ(statements[0]->getType(), ASTNodeType::INSERT_STATEMENT);
  }
}

TEST(ParserNewIntegrationTest, CreateTable) {
  MockParserNew parser("CREATE TABLE users (id INT, name VARCHAR);");
  auto statements = parser.parse();

  EXPECT_EQ(statements.size(), 1);
  if (statements.size() > 0) {
    EXPECT_EQ(statements[0]->getType(), ASTNodeType::CREATE_STATEMENT);
  }
}

TEST(ParserNewIntegrationTest, MultipleStatements) {
  MockParserNew parser(
  ...
  if (statements.size() > 0) {
    EXPECT_EQ(statements[0]->getType(), ASTNodeType::SELECT_STATEMENT);
    EXPECT_EQ(statements[1]->getType(), ASTNodeType::INSERT_STATEMENT);
  }
}
