...
struct MockToken {
    ...
    MockToken(MockTokenType t, const std::string& l, size_t ln = 1, size_t col = 1)
    ...
    bool isIdentifier() const {
        return type == MockTokenType::IDENTIFIER;
    }
    ...
    bool isOperator() const {
        return type == MockTokenType::EQUALS || 
               ...
               type == MockTokenType::LESS_THAN ||
               type == MockTokenType::AND ||
               type == MockTokenType::OR ||
               type == MockTokenType::NOT;
    }
    
private:
    std::string tokenTypeToString() const {
        switch (type) {
            case MockTokenType::SELECT: return "SELECT";
            case MockTokenType::INSERT: return "INSERT";
            case MockTokenType::UPDATE: return "UPDATE";
            case MockTokenType::DELETE: return "DELETE";
            case MockTokenType::CREATE: return "CREATE";
            case MockTokenType::DROP: return "DROP";
            case MockTokenType::ALTER: return "ALTER";
            case MockTokenType::FROM: return "FROM";
            case MockTokenType::WHERE: return "WHERE";
            case MockTokenType::INTO: return "INTO";
            case MockTokenType::SET: return "SET";
            case MockTokenType::VALUES: return "VALUES";
            case MockTokenType::TABLE: return "TABLE";
            case MockTokenType::DATABASE: return "DATABASE";
            case MockTokenType::INDEX: return "INDEX";
            case MockTokenType::IDENTIFIER: return "IDENTIFIER";
            case MockTokenType::NUMBER: return "NUMBER";
            case MockTokenType::STRING: return "STRING";
            case MockTokenType::ASTERISK: return "*";
            case MockTokenType::LEFT_PAREN: return "(";
            case MockTokenType::RIGHT_PAREN: return ")";
            case MockTokenType::COMMA: return ",";
            case MockTokenType::SEMICOLON: return ";";
            case MockTokenType::EQUALS: return "=";
            case MockTokenType::GREATER_THAN: return ">";
            case MockTokenType::LESS_THAN: return "<";
            case MockTokenType::AND: return "AND";
            case MockTokenType::OR: return "OR";
            case MockTokenType::NOT: return "NOT";
            case MockTokenType::PRIMARY_KEY: return "PRIMARY_KEY";
            case MockTokenType::FOREIGN_KEY: return "FOREIGN_KEY";
            case MockTokenType::VARCHAR: return "VARCHAR";
            case MockTokenType::INT: return "INT";
            case MockTokenType::FLOAT: return "FLOAT";
            case MockTokenType::BOOLEAN: return "BOOLEAN";
            case MockTokenType::END_OF_FILE: return "EOF";
...
public:
    ...
    MockToken nextToken() {
        ...
        if (std::isalpha(c) || c == '_') {
            size_t start = position_;
            position_++;
            while (position_ < input_.length() && (std::isalnum(input_[position_]) || input_[position_] == '_')) {
                position_++;
            }
        ...
        std::unordered_map<char, MockTokenType> singleTokens = {
            {'(', MockTokenType::LEFT_PAREN},
            {')', MockTokenType::RIGHT_PAREN},
            {',', MockTokenType::COMMA},
            {';', MockTokenType::SEMICOLON},
            {'=', MockTokenType::EQUALS},
            {'>', MockTokenType::GREATER_THAN},
...
private:
    void tokenize() {
        ...
        // Add EOF token
        tokens_.push_back(MockToken(MockTokenType::END_OF_FILE, "", line_, column_));
    }
...
int main() {
    ...
    try {
        ...
        std::vector<std::string> keywords = {"SELECT", "INSERT", "UPDATE", "DELETE", "CREATE", "DROP"};
        ...
        for (const auto& token : complexTokens) {
            if (token.isKeyword()) keywordCount++;
            else if (token.isIdentifier()) identifierCount++;
            else if (token.isLiteral()) literalCount++;
            else if (token.isOperator()) operatorCount++;
        }
        ...
        std::string commentSql = "SELECT * FROM users -- This is a comment\nWHERE id = 1;";
        ...
        std::cout << "✅ Tokens after comment handling: " << commentTokens.size() << std::endl;
        ...
        std::cout << "✅ Handled gracefully, tokens: " << errorTokens.size() << std::endl;
        
        // Test whitespace handling
        std::cout << "\n  9. Whitespace Handling Test" << std::endl;
        std::string whitespaceSql = "SELECT    *    FROM     users    WHERE   id   =   1;";
