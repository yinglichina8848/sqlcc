...
TEST_F(WhereClauseOptimizationTest, AndWhereCondition) {
  ...
  // 插入测试数据
  std::string insert1 =
      "INSERT INTO products (id, name, age) VALUES (1, 'Product1', 100);";
  sqlcc::sql_parser::ParserNew parser1(insert1);
  auto stmts1 = parser1.parse();
  executor.execute(std::move(stmts1[0]));

  std::string insert2 =
      "INSERT INTO products (id, name, age) VALUES (2, 'Product2', 200);";
  sqlcc::sql_parser::ParserNew parser2(insert2);
  auto stmts2 = parser2.parse();
  executor.execute(std::move(stmts2[0]));

  // TODO: 测试AND条件
  // std::string update_sql = "UPDATE products SET name = 'UpdatedProduct' WHERE
  // id = 1 AND age = 100;"; sqlcc::sql_parser::Parser parser3(update_sql); auto
  // stmts3 = parser3.parse(); auto result =
  // executor.execute(std::move(stmts3[0])); EXPECT_TRUE(result.success);
}
...
TEST_F(WhereClauseOptimizationTest, OrWhereCondition) {
  ...
  // TODO: 测试OR条件
  // std::string delete_sql = "DELETE FROM orders WHERE status = 'pending' OR
  // status = 'cancelled';"; sqlcc::sql_parser::Parser parser3(delete_sql); auto
  // stmts3 = parser3.parse(); auto result =
  // executor.execute(std::move(stmts3[0])); EXPECT_TRUE(result.success);
}
...
TEST_F(WhereClauseOptimizationTest, InClause) {
  ...
  // 插入测试数据
  for (int i = 1; i <= 5; i++) {
    std::string sql = "INSERT INTO items (id, category) VALUES (" +
                      std::to_string(i) + ", 'cat" + std::to_string(i % 2) +
                      "');";
...
TEST_F(WhereClauseOptimizationTest, BetweenClause) {
  ...
  // 插入测试数据
  for (int i = 1; i <= 10; i++) {
    std::string sql = "INSERT INTO transactions (id, amount) VALUES (" +
  ...
  // TODO: 测试BETWEEN操作符
  // std::string select_sql = "SELECT * FROM transactions WHERE amount BETWEEN
  // 300 AND 700;"; sqlcc::sql_parser::Parser parser(select_sql); auto stmts =
  // parser.parse(); auto result =
  // executor.execute(std::move(stmts[0])); EXPECT_TRUE(result.success);
}
...
TEST_F(WhereClauseOptimizationTest, LikeClause) {
  ...
  // TODO: 测试LIKE操作符
  // std::string select_sql = "SELECT * FROM users WHERE email LIKE
  // '%@example.com%';"; sqlcc::sql_parser::Parser parser3(select_sql); auto
  // stmts3 = parser3.parse(); auto result =
  // executor.execute(std::move(stmts3[0])); EXPECT_TRUE(result.success);
}
...
TEST_F(WhereClauseOptimizationTest, ComplexNestedConditions) {
  ...
  // 插入测试数据
  std::string insert1 = "INSERT INTO employees (id, name, age, department) "
                        "VALUES (1, 'Alice', 25, 'IT');";
  sqlcc::sql_parser::ParserNew parser1(insert1);
  auto stmts1 = parser1.parse();
  executor.execute(std::move(stmts1[0]));

  std::string insert2 = "INSERT INTO employees (id, name, age, department) "
                        "VALUES (2, 'Bob', 35, 'HR');";
  sqlcc::sql_parser::ParserNew parser2(insert2);
  auto stmts2 = parser2.parse();
  executor.execute(std::move(stmts2[0]));
  ...
  // TODO: 测试复杂条件
  // std::string select_sql = "SELECT * FROM employees WHERE (age >= 25 AND
  // department = 'IT') OR (age > 30 AND department = 'HR');";
