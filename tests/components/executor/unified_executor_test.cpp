...
TEST_F(UnifiedExecutorTest, UnifiedExecutorBasicTest) {
  // 测试CREATE DATABASE语句
  ParserNew create_db_parser("CREATE DATABASE test_db;");
  ...
  // 测试USE DATABASE语句
  ParserNew use_db_parser("USE test_db;");
...
TEST_F(UnifiedExecutorTest, ExecutionContextUpdateTest) {
  // 执行一条SQL语句
  ParserNew create_db_parser("CREATE DATABASE test_context;");
  auto create_db_stmts = create_db_parser.parse();
  ASSERT_FALSE(create_db_stmts.empty());
  auto create_db_stmt = std::move(create_db_stmts[0]);
...
TEST_F(UnifiedExecutorTest, StrategyPatternImprovementTest) {
  ...
  ParserNew create_db_parser("CREATE DATABASE test_strategy;");
  auto create_db_stmts = create_db_parser.parse();
  ASSERT_FALSE(create_db_stmts.empty());
  ...
  ParserNew select_parser("SELECT * FROM test_table;");
