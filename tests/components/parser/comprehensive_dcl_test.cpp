...
public:
  ...
  std::string toString() const override {
    ...
    oss << " ON " << objectType_ << " " << objectName_ << " TO " << grantee_;
    return oss.str();
  }
};
...
public:
  ...
  std::string toString() const override {
    ...
    oss << " ON " << objectType_ << " " << objectName_ << " FROM " << grantee_;
    return oss.str();
  }
};
...
public:
  enum ShowType { DATABASES, TABLES, CREATE_TABLE, COLUMNS, INDEXES, GRANTS };
...
public:
  ...
  void setFromDatabase(const std::string &db) {
    fromDatabase_ = db;
    hasFromDb_ = true;
  }
...
int main() {
  std::cout << "=== SQLCC 完整DCL功能测试 ===" << std::endl;
  std::cout << "包含：CREATE USER、DROP USER、GRANT、REVOKE、SHOW GRANTS等"
            << std::endl;
