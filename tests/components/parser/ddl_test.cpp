...
protected:
  void SetUp() override {
    // 创建数据目录（使用绝对路径避免build目录下的路径问题）
    std::string test_data_dir = "test_data";
    fs::create_directories(test_data_dir);
    // 创建数据库管理器，使用新版本的路径结构
    std::string db_path = test_data_dir + "/sqlcc.db";
    db_manager = std::make_shared<sqlcc::DatabaseManager>(db_path);
    // 创建DDL执行器
...
TEST_F(DDLTest, CreateDatabase) {
  ...
  EXPECT_NE(result.getMessage().find("created successfully"),
            std::string::npos);
}
...
TEST_F(DDLTest, DropTable) {
  ...
  // 检查结果
  EXPECT_EQ(result.getStatus(), sqlcc::ExecutionResult::SUCCESS);
  EXPECT_NE(result.getMessage().find("dropped successfully"),
            std::string::npos);
}
