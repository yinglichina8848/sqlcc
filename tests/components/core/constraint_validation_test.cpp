...
TEST_F(ConstraintValidationTest, PrimaryKeyConstraintTest) {
  ...
  sqlcc::sql_parser::ParserNew parser1(insert_sql1);
  ...
  sqlcc::sql_parser::ParserNew parser2(insert_sql2);
...
TEST_F(ConstraintValidationTest, NotNullConstraintTest) {
  ...
  sqlcc::sql_parser::ParserNew parser(insert_sql);
...
TEST_F(ConstraintValidationTest, UniqueConstraintTest) {
  ...
  // 第一次插入
  std::string insert_sql1 =
      "INSERT INTO accounts (id, email) VALUES (1, 'alice@example.com');";
  sqlcc::sql_parser::ParserNew parser1(insert_sql1);
  auto stmts1 = parser1.parse();
  executor.execute(std::move(stmts1[0]));

  // TODO: 第二次插入相同email应该失败
  std::string insert_sql2 =
      "INSERT INTO accounts (id, email) VALUES (2, 'alice@example.com');";
  sqlcc::sql_parser::ParserNew parser2(insert_sql2);
  auto stmts2 = parser2.parse();
  auto result2 = executor.execute(std::move(stmts2[0]));
