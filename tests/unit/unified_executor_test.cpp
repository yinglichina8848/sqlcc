...
TEST_F(UnifiedExecutorTest, StrategyPatternImprovementTest) {
  ...
  std::vector<std::string> sqls = {
      "CREATE DATABASE test_strategy;",
      "USE test_strategy;",
      "CREATE TABLE strategy_test (id INTEGER, name VARCHAR);",
      "INSERT INTO strategy_test VALUES (1, 'test');",
      "SELECT * FROM strategy_test;",
      "UPDATE strategy_test SET name = 'updated' WHERE id = 1;",
      "DELETE FROM strategy_test WHERE id = 1;",
      "DROP TABLE strategy_test;",
      "DROP DATABASE test_strategy;"};

  for (const auto &sql : sqls) {
    Parser parser(sql);
    auto stmts = parser.parseStatements();
    if (!stmts.empty()) {
      auto result = unified_executor_->execute(std::move(stmts[0]));
      // 大部分语句应该执行成功
      EXPECT_TRUE(result.success)
          << "SQL: " << sql << " failed: " << result.message;
    }
  }
}
...
TEST_F(UnifiedExecutorTest, UnifiedPermissionCheckTest) {
  ...
  auto create_user_stmts = create_user_parser.parseStatements();
  ASSERT_FALSE(create_user_stmts.empty());
  auto create_user_stmt = std::move(create_user_stmts[0]);

  auto result = unified_executor_->execute(std::move(create_user_stmt));
  EXPECT_TRUE(result.success);

  // 测试GRANT语句（需要管理员权限）
  Parser grant_parser("GRANT SELECT ON TABLE test_table TO test_user;");
  auto grant_stmts = grant_parser.parseStatements();
  ASSERT_FALSE(grant_stmts.empty());
  auto grant_stmt = std::move(grant_stmts[0]);

  result = unified_executor_->execute(std::move(grant_stmt));
  EXPECT_TRUE(result.success);
}
