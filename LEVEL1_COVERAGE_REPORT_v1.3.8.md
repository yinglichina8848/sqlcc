# Level 1 Foundation 覆盖率测试报告 v1.3.8
...
成功完成了 SQLCC v1.3.8 Level 1 Foundation 核心组件的覆盖率测试，使用 LLVM Coverage 工具收集并分析了代码覆盖率数据。所有测试均使用真实实现，覆盖率数据反映了实际的代码执行情况。
...
- **HTML 覆盖率报告**: `/home/liying/sqlcc/coverage_html_report_level1_v1.3.8/`
- **原始覆盖率数据**:
...
2. 收集覆盖率数据:
   ```bash
   find bazel-out/k8-fastbuild -name "types_test*.profraw"
   ```

3. 合并到总覆盖率数据:
   ```bash
   llvm-profdata merge -o merged_coverage_level1.profdata \
     merged_coverage_level1.profdata \
     <types_test.profraw>
   ```
...
**审核状态**: 已完成  
**下次更新**: Config 和 Types 模块覆盖率修复完成后
