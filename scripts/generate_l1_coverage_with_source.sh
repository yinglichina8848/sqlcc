...
# SQLCC Level 1 覆盖率报告生成脚本 - 包含核心源码覆盖率
# 使用 llvm-cov 从 object 文件生成覆盖率报告

set -e

PROJECT_ROOT="/home/liying/sqlcc"
COVERAGE_DIR="$PROJECT_ROOT/coverage_report_l1_with_source"
BAZEL_CACHE="/home/liying/.cache/bazel/_bazel_liying/68dbc53c53085b82ed46643b8af8ae0d/execroot/_main"

echo "=========================================="
echo "SQLCC Level 1 覆盖率报告生成 (含源码)"
echo "=========================================="

mkdir -p "$COVERAGE_DIR"

# 模块配置: 测试目标 -> 源文件目录 -> object 文件模式
declare -A MODULES=(
    ["types"]="src/types:bazel-out/k8-fastbuild/bin/src/types/_objs/types_coverage/*.pic.o"
    ["config"]="src/utils:bazel-out/k8-fastbuild/bin/src/utils/_objs/utils/*.pic.o"
    ["logger"]="src/logger:bazel-out/k8-fastbuild/bin/src/logger/_objs/logger/*.pic.o"
    ["exception"]="src/exception:bazel-out/k8-fastbuild/bin/src/exception/_objs/exception/*.pic.o"
)

echo "步骤 1: 生成各模块源码覆盖率报告..."
...
for MODULE in "${!MODULES[@]}"; do
    ...
    # 收集 object 文件
    OBJ_FILES=$(find "$BAZEL_CACHE/$OBJ_PATTERN" -name "*.pic.o" 2>/dev/null | tr '\n' ' ')
    ...
    # 收集源文件
    SOURCE_FILES=$(find "$PROJECT_ROOT/$SOURCE_DIR" -name "*.cpp" 2>/dev/null | tr '\n' ' ')

    # 查找覆盖率数据
    COVERAGE_DIR_PATH=$(find "$BAZEL_CACHE" -path "*_coverage*" -name "*.profraw" 2>/dev/null | head -1 | xargs dirname)

    if [ -n "$COVERAGE_DIR_PATH" ] && [ -d "$COVERAGE_DIR_PATH" ]; then
        # 合并覆盖率数据
        PROFDATA_FILE="$OUTPUT_DIR/$MODULE.profdata"
        llvm-profdata-20 merge -o "$PROFDATA_FILE" "$COVERAGE_DIR_PATH"/*.profraw 2>/dev/null || true
...
# 收集所有 profdata 文件
PROFDATA_FILES=$(find "$COVERAGE_DIR" -name "*.profdata" 2>/dev/null | tr '\n' ' ')

if [ -n "$PROFDATA_FILES" ]; then
    # 合并所有覆盖率数据
    llvm-profdata-20 merge -o "$COVERAGE_DIR/merged.profdata" $PROFDATA_FILES 2>/dev/null || true
fi

echo ""
echo "步骤 3: 生成 HTML 索引页..."
...
<head>
    ...
    <title>SQLCC v1.3.9 Level 1 覆盖率报告 (含源码)</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .module-item { display: flex; align-items: center; padding: 18px; background: #21262d; border-radius: 12px; transition: all 0.3s; }
        .module-item:hover { background: rgba(0,217,255,0.1); transform: translateX(5px); }
        .module-icon { width: 50px; height: 50px; background: linear-gradient(135deg, #238636, #3fb950); border-radius: 15px; display: flex; align-items: center; justify-content: center; font-size: 1.5em; margin-right: 20px; }
        .module-info { flex: 1; }
        .module-name { font-size: 1.1em; font-weight: bold; margin-bottom: 5px; }
...
<body>
    <div class="container">
        ...
        <div class="grid">
            ...
            <div class="card">
                <h2>🛠️ 实现验证</h2>
                <p style="margin-bottom: 15px; color: #8b949e;">所有测试使用<strong style="color: #3fb950;">真实实现</strong>，覆盖率统计包含核心源码文件 (.cpp)</p>
                <div class="verify-list">
                    <div class="verify-item">
                        <span class="verify-icon">✓</span>
                        <span>//src/types (domain_manager.cpp)</span>
                    </div>
                    <div class="verify-item">
                        <span class="verify-icon">✓</span>
                        <span>//src/utils (config_manager.cpp 等)</span>
                    </div>
                    <div class="verify-item">
                        <span class="verify-icon">✓</span>
                        <span>//src/logger (logger.cpp)</span>
                    </div>
                    <div class="verify-item">
                        <span class="verify-icon">✓</span>
                        <span>//src/exception (exception.cpp)</span>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="module-list">
                <div class="module-item">
                    ...
                    <div class="module-info">
                        ...
                        <div class="module-desc">domain_manager.cpp | 42 测试</div>
                    </div>
                ...
                <div class="module-item">
                    ...
                    <div class="module-info">
                        ...
                        <div class="module-desc">config_manager.cpp 等 | 29 测试</div>
                    </div>
                ...
                <div class="module-item">
                    ...
                    <div class="module-info">
                        ...
                        <div class="module-desc">logger.cpp | 20 测试</div>
                    </div>
                ...
                <div class="module-item">
                    ...
                    <div class="module-info">
                        ...
                        <div class="module-desc">exception.cpp | 32 测试</div>
                    </div>
        ...
        <div class="card">
            <h2>📈 覆盖率示例 (domain_manager.cpp)</h2>
            <pre style="background: #0d1117; padding: 20px; border-radius: 10px; overflow-x: auto; font-size: 12px;">
   ...
   28|     1|}
        ...
        <div class="footer">
            <p>🎉 SQLCC v1.3.9 Level 1 测试覆盖率报告 (含源码)</p>
...
echo ""
echo "📁 生成的文件:"
find "$COVERAGE_DIR" -type f -name "*.html" 2>/dev/null | head -10
echo ""
echo "🌐 查看报告:"
