...
# 测试工作目录配置
TEST_WORKING_DIR="build_test"  # 专门的测试工作目录，使用新的build_test目录
...
# 保存原始目录
ORIGINAL_DIR=$(pwd)
# 使用现有的build_new目录，不清理，只确保必要的目录结构存在
mkdir -p "$TEST_WORKING_DIR" 2>/dev/null
mkdir -p "$TEST_WORKING_DIR/build" 2>/dev/null
# 设置缓存目录到测试工作目录中
...
# 切换到构建目录
cd "$TEST_WORKING_DIR" || { echo -e "${RED}无法进入测试工作目录${NC}"; exit 1; }
...
# 检查CMake配置是否需要更新
 echo -e "${BLUE}检查CMake配置...${NC}"
 
 # 清理现有的CMake文件，使用正确的源目录重新生成
 echo -e "${BLUE}清理现有的CMake文件...${NC}"
...
# 开始编译测试
 ...
 # 检查是否存在Makefile
 if [ ! -f "Makefile" ]; then
     echo -e "${RED}错误: Makefile不存在，无法编译测试${NC}"
     echo -e "${BLUE}尝试重新运行CMake配置...${NC}"
     # 重新运行CMake配置
     cmake .. || {
         echo -e "${RED}CMake配置失败，请检查CMakeLists.txt文件${NC}"
         exit 1
     }
 fi
 
 # 编译测试
 echo -e "${BLUE}编译所有测试...${NC}"
 ...
 find . -type f -executable 2>/dev/null | grep -v "CMakeFiles" | sort
