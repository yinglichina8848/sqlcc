...
- 在尝试本地用 `--config=asan` 构建测试目标时，暴露并修复了一些 BUILD/包含路径相关问题（例如 `//src:execution_engine` 的目标和头文件包含路径问题），但构建过程中仍发现若干头文件/依赖声明不一致导致的编译失败。
...
## 10. 结论与下一步建议
- 总体结论: SQLCC 在教学场景下已实现核心模块且设计较为清晰，但在工程化（构建一致性、自动化检测）、内存/资源安全与并发健壮性方面存在不足，需要以 Sanitizer 为中心的短期修复以及中长期的并发&事务模型改进。
