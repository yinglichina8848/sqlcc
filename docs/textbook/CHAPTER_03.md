...
**实际应用示例**: 向量空间在语义搜索中的应用

```
用户资料表 (users) 的向量空间表示:

原始关系:
users(user_id, name, age, interests, location)
记录: [1, "张三", 25, "篮球,阅读,编程", "北京"]
      [2, "李四", 30, "足球,电影,音乐", "上海"]
      [3, "王五", 28, "篮球,旅行,摄影", "广州"]

向量化后:
users_vectors = [
    [1, 0.1, 0.8, 0.2, 0.3, 0.9, 0.5, 0.2, 0.7],  // 张三的兴趣+年龄向量
    [2, 0.4, 0.3, 0.0, 0.8, 0.6, 0.9, 0.1, 0.3],  // 李四的兴趣+年龄向量
    [3, 0.2, 0.8, 0.7, 0.9, 0.1, 0.4, 0.2, 0.8]   // 王五的兴趣+年龄向量
]

相似性搜索查询:
传统SQL: SELECT * FROM users WHERE interests LIKE '%篮球%'
向量搜索: SELECT * FROM users ORDER BY cosine_similarity(interests_vector, [0,1,0,0]) DESC

查询结果:
- 张三: 余弦相似度 = 0.92 (有篮球兴趣)
- 王五: 余弦相似度 = 0.87 (有篮球兴趣)
- 李四: 余弦相似度 = 0.23 (无篮球兴趣，相似度低)
```
