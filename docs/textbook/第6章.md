# 《数据库系统原理与开发实践》 - 第6章：存储引擎与执行器：统一执行策略与ACID工程实践

**从AST执行到物理存储：策略模式、统一执行器与事务管理的深度集成**

---

## 🎯 **本章核心目标**

深入理解SQLCC执行引擎与存储引擎的协同工作：
- **UnifiedExecutor统一执行器**：执行策略的集成与协调机制
- **ExecutionStrategy执行策略模式**：DDL、DML、DCL的具体实现
- **AggregateEngine聚合引擎**：GROUP BY、聚合函数的工程实现
- **StorageEngine存储引擎**：逻辑表到物理文件的映射
- **TransactionManager事务管理器**：ACID特性的工程保证
- **LockManager锁管理器**：并发控制与死锁检测机制

---

## 6.2 SQLCC统一执行器：执行策略的集成与协调

`UnifiedExecutor`是SQLCC执行引擎的核心，它作为统一入口，协调各种执行策略的运行。通过组合模式和策略模式的应用，`UnifiedExecutor`实现了灵活的SQL语句执行架构。

### 6.2.1 UnifiedExecutor的设计哲学

`UnifiedExecutor`体现了现代软件工程的几个核心设计理念：

**1. 单一职责原则**
- `UnifiedExecutor`专注于执行策略的协调和调度
- 具体的执行逻辑委托给专门的执行策略类

**2. 开闭原则**
- 新增执行策略通过扩展现有类实现
- 无需修改`UnifiedExecutor`的核心逻辑

**3. 依赖倒置原则**
- `UnifiedExecutor`依赖`ExecutionStrategy`抽象接口
- 不依赖具体的执行策略实现

**4. 组合模式的应用**
- `UnifiedExecutor`组合了多个执行引擎（聚合引擎、分组执行器等）
- 这些执行引擎可以独立开发和测试

### 6.2.2 UnifiedExecutor的架构设计
...
classDiagram
    class UnifiedExecutor {
        -aggregate_engine_ AggregateEngine
        -group_by_executor_ GroupByExecutor
        -cost_estimator_ CostEstimator
        +execute(stmt, context) ExecutionResult
        +checkPermission(stmt, context) bool
        +validate(stmt, context) bool
        -executeSelect(stmt, context) ExecutionResult
        -executeInsert(stmt, context) ExecutionResult
        -executeUpdate(stmt, context) ExecutionResult
        -executeDelete(stmt, context) ExecutionResult
        -requiresAggregation(stmt) bool
        -requiresGrouping(stmt) bool
        -canUseSimpleExecution(stmt) bool
    }
    ...
    class AggregateEngine {
        +executeAggregateQuery(stmt, context) ExecutionResult
        -processGroupBy(stmt, context) ExecutionResult
        -processSimpleAggregate(stmt, context) ExecutionResult
    }
    
    class GroupByExecutor {
        +executeGroupBy(stmt, context) ExecutionResult
        -processGrouping(stmt, context) ExecutionResult
        -applyAggregatesToGroups(stmt, groups, context) ExecutionResult
    }
    
    class CostEstimator {
        +optimizeQuery(stmt, context) QueryPlan
        +estimateCost(plan) double
        +selectOptimalPlan(plans) QueryPlan
    }
    
    ExecutionStrategy <|-- UnifiedExecutor
    UnifiedExecutor *-- AggregateEngine
    UnifiedExecutor *-- GroupByExecutor
    UnifiedExecutor *-- CostEstimator
```

### 6.2.3 UnifiedExecutor的核心实现

**源码位置**: `/home/liying/sqlcc/src/execution/unified_executor.h`

```cpp
// SQLCC UnifiedExecutor核心实现
class UnifiedExecutor : public ExecutionStrategy {
public:
    UnifiedExecutor() 
        : aggregate_engine_(std::make_unique<AggregateEngine>()),
          group_by_executor_(std::make_unique<GroupByExecutor>()),
          cost_estimator_(std::make_unique<CostEstimator>()) {}
    ...
    bool checkPermission(
        ...
        return validator.validate(stmt, context);
    }
...
private:
    ...
    // 判断是否需要聚合
    bool requiresAggregation(const SelectStatement& stmt) const {
        // 检查SELECT列表中是否有聚合函数
    ...
    // 判断是否需要分组
    bool requiresGrouping(const SelectStatement& stmt) const {
        // 检查是否有GROUP BY子句
        return !stmt.getGroupBy().empty();
    }
    
    // 判断是否可以使用简单执行
    bool canUseSimpleExecution(const SelectStatement& stmt) const {
        return !requiresAggregation(stmt) && !requiresGrouping(stmt);
    }
};
```

### 6.2.4 AggregateEngine聚合引擎的详细实现

`AggregateEngine`负责处理包含聚合函数（SUM、AVG、COUNT、MIN、MAX）的查询：

```cpp
// SQLCC AggregateEngine实现
class AggregateEngine {
public:
    ...
    // 执行聚合查询
    ExecutionResult executeAggregateQuery(
        const SelectStatement& stmt, ExecutionContext& context) {
        
        // 识别聚合函数
        std::vector<AggregateFunction> aggFuncs = 
            identifyAggregateFunctions(stmt);
        
        // 判断是否需要分组
        if (requiresGrouping(stmt)) {
            // 分组聚合
            return processGroupBy(stmt, context);
        } else {
            // 简单聚合
            return processSimpleAggregate(stmt, context);
        }
    }
...
private:
    ...
    using AggregateResults = 
        std::unordered_map<GroupKey, std::vector<AggregateResult>>;
    
    // 处理分组聚合
    ExecutionResult processGroupBy(
        const SelectStatement& stmt, ExecutionContext& context) {
        
        // 1. 扫描数据并分组
        AggregateResults groups;
        scanDataAndGroup(stmt, context, groups);
        
        // 2. 对每个分组应用聚合函数
        ExecutionResult result;
        for (auto& [groupKey, aggResults] : groups) {
            for (auto& aggResult : aggResults) {
                std::shared_ptr<void> finalValue = 
                    finalizeAggregateResult(aggResult);
                addResultToOutput(result, finalValue);
            }
        }
    ...
    // 处理简单聚合
    ExecutionResult processSimpleAggregate(
        const SelectStatement& stmt, ExecutionContext& context) {
        ...
        // 2. 初始化聚合结果
        std::vector<AggregateResult> aggResults;
        for (auto func : aggFuncs) {
            AggregateResult aggResult;
            aggResult.function = func;
            aggResult.count = 0;
            aggResults.push_back(aggResult);
        }
        
        // 3. 扫描数据并更新聚合结果
        scanDataAndAggregate(stmt, context, aggResults);
    ...
    // 识别聚合函数
    std::vector<AggregateFunction> identifyAggregateFunctions(
        ...
        for (const auto& expr : stmt.getSelectList()) {
            if (auto funcCall = 
                dynamic_cast<FunctionCallExpression*>(expr.get())) {
                std::string funcName = funcCall->getFunctionName();
                
                if (funcName == "COUNT") {
                    aggFuncs.push_back(AggregateFunction::COUNT);
                } else if (funcName == "SUM") {
                    aggFuncs.push_back(AggregateFunction::SUM);
                } else if (funcName == "AVG") {
                    aggFuncs.push_back(AggregateFunction::AVG);
                } else if (funcName == "MIN") {
                    aggFuncs.push_back(AggregateFunction::MIN);
                } else if (funcName == "MAX") {
                    aggFuncs.push_back(AggregateFunction::MAX);
                }
            }
        }
    ...
    // 计算最终聚合结果
    std::shared_ptr<void> finalizeAggregateResult(
        const AggregateResult& result) {
        
        switch (result.function) {
            case AggregateFunction::COUNT:
                return std::make_shared<int64_t>(result.count);
            case AggregateFunction::AVG:
                // 计算平均值
                break;
            default:
...
### 6.2.5 GroupByExecutor分组执行器的详细实现

`GroupByExecutor`负责处理GROUP BY子句，将数据按照指定列进行分组：

```cpp
// SQLCC GroupByExecutor实现
class GroupByExecutor {
public:
    ...
    // 执行GROUP BY查询
    ExecutionResult executeGroupBy(
        const SelectStatement& stmt, ExecutionContext& context) {
        
        // 1. 执行分组
        ExecutionResult groupingResult = 
            processGrouping(stmt, context);
        
        // 2. 应用聚合函数
        ExecutionResult finalResult = 
            applyAggregatesToGroups(stmt, groupingResult.getGroups(), context);
...
private:
    // 分组键和数据
    using GroupKey = std::vector<std::shared_ptr<void>>;
    using GroupData = std::vector<std::unordered_map<std::string, 
                                                std::shared_ptr<void>>>;
    
    // 处理分组
    ExecutionResult processGrouping(
        const SelectStatement& stmt, ExecutionContext& context) {
        
        ExecutionResult result;
        std::unordered_map<GroupKey, GroupData> groups;
        
        // 1. 扫描数据
        auto dataStream = scanData(stmt, context);
    ...
    // 应用聚合函数到分组
    ExecutionResult applyAggregatesToGroups(
        const SelectStatement& stmt,
        const std::unordered_map<GroupKey, GroupData>& groups,
        ExecutionContext& context) {
        
        ExecutionResult result;
        AggregateEngine aggEngine;
        
        // 对每个分组应用聚合函数
        for (const auto& [groupKey, groupData] : groups) {
            // 计算聚合结果
            auto aggResults = aggEngine.computeAggregates(
                stmt.getSelectList(), groupData);
            
            // 构建输出行
            std::unordered_map<std::string, std::shared_ptr<void>> outputRow;
            
            // 添加分组键
            size_t keyIndex = 0;
            for (const auto& expr : stmt.getGroupBy()) {
                outputRow[expr->toString()] = groupKey[keyIndex++];
            }
            
            // 添加聚合结果
            for (const auto& aggResult : aggResults) {
                outputRow[aggResult.column_name] = aggResult.value;
            }
    ...
    GroupKey createGroupKey(
        const std::vector<sql_parser::Expression*>& groupByExprs,
        const std::unordered_map<std::string, std::shared_ptr<void>>& rowData) {
        
        GroupKey groupKey;
        
        for (const auto& expr : groupByExprs) {
            // 计算分组表达式的值
            auto value = evaluateExpression(expr, rowData);
            groupKey.push_back(value);
        }
    ...
    size_t computeGroupKeyHash(const GroupKey& key) const {
        ...
        for (const auto& value : key) {
            hash = hash * 31 + computeValueHash(value);
        }
...
`UnifiedExecutor`通过智能的策略选择机制，为不同类型的查询选择最优的执行策略：

```
mermaid
flowchart TD
    A[SELECT语句输入] --> B{是否有GROUP BY?}
    B -->|是| C{是否有聚合函数?}
    B -->|否| D{是否有聚合函数?}
    
    C -->|是| E[使用GroupByExecutor]
    C -->|否| F[使用简单分组]
    
    D -->|是| G[使用AggregateEngine]
    D -->|否| H{是否有JOIN?}
    
    H -->|是| I[使用JoinExecutor]
    H -->|否| J[使用简单执行]
...
**2. 代码复用最大化**
- 公共的执行逻辑在`UnifiedExecutor`中实现
- 具体策略只关注自身特有的执行逻辑

**3. 可扩展性增强**
- 新增执行策略只需实现`ExecutionStrategy`接口
- 新增执行引擎只需组合到`UnifiedExecutor`中
...
**5. 可测试性改进**
- 每个执行策略可以独立测试
- `UnifiedExecutor`的协调逻辑可以单独验证

通过`UnifiedExecutor`统一执行器的设计，SQLCC实现了灵活、高效、可扩展的SQL语句执行架构。这种架构不仅满足了当前的功能需求，也为未来的性能优化和功能扩展奠定了坚实的基础。

---

## 6.3 存储引擎的体系架构：逻辑与物理的桥梁
