...
## 8.5 分布式数据库架构与实现：从单机到集群的演进

随着数据量的爆炸式增长和全球化业务的发展，单机数据库已经无法满足现代应用的需求。分布式数据库系统通过将数据分布在多个节点上，实现了横向扩展、高可用性和地理分布能力。

### 8.5.1 分布式系统基础理论

#### CAP定理

分布式系统的三大基本属性：一致性（Consistency）、可用性（Availability）、分区容错性（Partition Tolerance）。
...
- 可用性：每次请求都能获得响应（不保证是最新的数据）
- 分区容错性：系统在网络分区时仍能继续运行

现实约束：无法同时满足C、A、P三个属性，最多只能满足两个
```

#### 一致性模型

**强一致性**：
- 线性一致性：所有操作看起来像是原子地同时发生
- 顺序一致性：所有进程看到相同的操作顺序
- 因果一致性：有因果关系的操作保持顺序

**弱一致性**：
- 最终一致性：系统保证在没有新更新的情况下，最终所有副本将一致
- 读己之写：进程总是能看到自己的写操作
- 会话一致性：会话内的操作保持一致性
...
SAGA将长事务分解为多个本地事务：
- 每个本地事务都有对应的补偿事务
- 如果某个本地事务失败，执行之前的补偿事务
- 通过补偿机制实现最终一致性
```

### 8.5.2 SQLCC分布式架构设计

SQLCC的分布式架构基于微服务设计理念，将不同功能模块独立部署，实现了灵活的扩展和高可用性。
...
```cpp
// SQLCC QueryRouter实现
// 源码位置: /home/liying/sqlcc/src/distributed/query_router.h
...
public:
    ...
    QueryRouteResult routeQuery(const SelectStatement& query) {
        ...
        if (isSingleShardQuery(involvedShards)) {
            // 单分片查询：路由到单个节点
            result.strategy = QueryStrategy::SINGLE_SHARD;
            result.targetNodes = getNodesForShard(involvedShards[0]);
        } else if (canUseShardJoin(involvedShards)) {
            // 分片连接查询：使用分布式连接
            result.strategy = QueryStrategy::DISTRIBUTED_JOIN;
            result.targetNodes = getAllNodes();
            result.joinPlan = buildDistributedJoinPlan(query);
        } else {
            // 复杂查询：使用全局聚合
            result.strategy = QueryStrategy::GLOBAL_AGGREGATE;
            result.targetNodes = getAllNodes();
            result.aggregationPlan = buildAggregationPlan(query);
        }
...
private:
    ...
    std::vector<ShardID> analyzeQueryShards(const SelectStatement& query) {
        ...
        // 检查FROM子句中的表
        for (const auto& table : query.getFromTables()) {
            auto tableShards = getShardsForTable(table);
            shards.insert(shards.end(), tableShards.begin(), tableShards.end());
        }
    ...
    bool canUseShardJoin(const std::vector<ShardID>& shards) {
        // 简化判断：如果涉及两个分片且分片键相同
        return shards.size() == 2 && shards[0].partitionKey == shards[1].partitionKey;
    }
};
```

**2. 分布式事务协调器（DistributedTransactionCoordinator）**

```cpp
// SQLCC DistributedTransactionCoordinator实现
// 源码位置: /home/liying/sqlcc/src/distributed/transaction_coordinator.h
...
private:
    ...
    PrepareResult prepareTransaction(const std::vector<TransactionRequest>& requests) {
        ...
        for (const auto& request : requests) {
            // 发送准备请求到参与节点
            PrepareResponse response = sendPrepareRequest(request.nodeId, request);
            
            if (response.success) {
                result.preparedNodes.push_back(request.nodeId);
            } else {
                result.failedNodes.push_back(request.nodeId);
    ...
    CommitResult commitTransaction(const std::vector<TransactionRequest>& requests) {
        ...
        for (const auto& request : requests) {
            // 发送提交请求到参与节点
            CommitResponse response = sendCommitRequest(request.nodeId, request);
            
            if (response.success) {
                result.committedNodes.push_back(request.nodeId);
            } else {
                result.failedNodes.push_back(request.nodeId);
                result.allCommitted = false;
            }
        }
...
数据分片是分布式数据库的核心技术，决定了数据如何在多个节点之间分布。

#### 分片策略类型

**1. 水平分片（Range Partitioning）**
```cpp
// 基于范围的水平分片
class RangeShardingStrategy {
public:
    ShardID getShard(const std::string& key) {
        // 假设key是数值类型
...
```cpp
// 一致性哈希分片策略
class ConsistentHashShardingStrategy {
public:
    void addNode(ShardID shardId, int virtualNodes = 100) {
        // 为每个物理节点创建多个虚拟节点
        for (int i = 0; i < virtualNodes; ++i) {
            std::string virtualNodeKey = 
                shardId.toString() + "#" + std::to_string(i);
            
            size_t hashValue = std::hash<std::string>{}(virtualNodeKey);
            ring_[hashValue] = shardId;
        }
    }
    
    void removeNode(ShardID shardId) {
        // 移除节点的所有虚拟节点
        auto it = ring_.begin();
        while (it != ring_.end()) {
    ...
    ShardID getShard(const std::string& key) {
        ...
        // 在环上查找第一个大于等于hashValue的节点
        auto it = ring_.lower_bound(hashValue);
...
private:
    std::map<size_t, ShardID> ring_; // 哈希环
};
```

### 8.5.4 分布式查询优化

分布式查询优化是提升分布式数据库性能的关键技术。

#### 分布式查询计划生成

```cpp
// SQLCC DistributedQueryOptimizer实现
// 源码位置: /home/liying/sqlcc/src/distributed/query_optimizer.h

class DistributedQueryOptimizer {
public:
    DistributedQueryPlan optimizeQuery(const SelectStatement& query) {
        DistributedQueryPlan plan;
        
        // 1. 分析查询涉及的分片
        auto involvedShards = analyzeInvolvedShards(query);
        
        // 2. 选择查询执行策略
        QueryExecutionStrategy strategy = selectExecutionStrategy(involvedShards, query);
        
        // 3. 生成分片子查询
        auto subQueries = generateSubQueries(query, involvedShards);
        
        // 4. 优化子查询执行顺序
        auto optimizedOrder = optimizeSubQueryOrder(subQueries);
        
        // 5. 设计数据收集和聚合策略
        auto aggregationPlan = designAggregationStrategy(query, subQueries);
        
        plan.strategy = strategy;
        plan.subQueries = subQueries;
        plan.executionOrder = optimizedOrder;
        plan.aggregationPlan = aggregationPlan;
...
private:
    // 选择查询执行策略
    QueryExecutionStrategy selectExecutionStrategy(
        const std::vector<ShardID>& shards,
        const SelectStatement& query) {
        
        if (shards.size() == 1) {
            // 单分片：直接执行
            return QueryExecutionStrategy::DIRECT;
        } else if (isBroadcastJoin(query)) {
            // 广播连接：将小表广播到所有节点
            return QueryExecutionStrategy::BROADCAST_JOIN;
        } else if (isShuffleJoin(query)) {
            // 洗牌连接：根据连接键重新分布数据
            return QueryExecutionStrategy::SHUFFLE_JOIN;
        } else if (isGatherThenAggregate(query)) {
            // 收集后聚合：先收集所有数据再聚合
            return QueryExecutionStrategy::GATHER_AGGREGATE;
        } else {
            // 默认：分布式执行
            return QueryExecutionStrategy::DISTRIBUTED;
        }
    }
    
    // 检查是否是广播连接
    bool isBroadcastJoin(const SelectStatement& query) {
        // 判断是否有小表可以广播
        auto smallTables = identifySmallTables(query);
        return !smallTables.empty() && smallTables.size() == 1;
    }
    
    // 检查是否是洗牌连接
    bool isShuffleJoin(const SelectStatement& query) {
        // 判断是否可以根据连接键重新分布
        auto joinKeys = extractJoinKeys(query);
        return !joinKeys.empty() && joinKeys.size() == 1;
    }
};
```
...
分布式系统必须具备强大的故障检测和恢复能力。

#### 故障检测

```cpp
// SQLCC FailureDetector实现
// 源码位置: /home/liying/sqlcc/src/distributed/failure_detector.h

class FailureDetector {
public:
    // 健康检查
    HealthStatus checkNodeHealth(NodeID nodeId) {
        HealthStatus status;
        
        // 1. 检查节点是否响应
        bool isAlive = pingNode(nodeId);
        ...
        // 2. 检查节点负载
        status.cpuUsage = getCpuUsage(nodeId);
        status.memoryUsage = getMemoryUsage(nodeId);
        status.diskUsage = getDiskUsage(nodeId);
        
        // 3. 检查节点响应时间
        status.responseTime = measureResponseTime(nodeId);
        
        // 4. 判断节点是否过载
        status.isOverloaded = (status.cpuUsage > 80.0 || 
                              status.memoryUsage > 80.0 ||
                              status.diskUsage > 90.0);
        
        return status;
    }
    
    // 检测网络分区
    PartitionStatus detectNetworkPartition() {
        PartitionStatus status;
        
        // 收集所有节点的网络视图
        auto networkViews = collectNetworkViews();
        
        // 分析视图是否一致
        if (areViewsConsistent(networkViews)) {
        ...
        status.partitionGroups = identifyPartitionGroups(networkViews);
        
        return status;
    }
    
private:
    bool pingNode(NodeID nodeId);
    double getCpuUsage(NodeID nodeId);
    double getMemoryUsage(NodeID nodeId);
    double getDiskUsage(NodeID nodeId);
    double measureResponseTime(NodeID nodeId);
    std::vector<NetworkView> collectNetworkViews();
    bool areViewsConsistent(const std::vector<NetworkView>& views);
    std::vector<std::vector<NodeID>> identifyPartitionGroups(
        const std::vector<NetworkView>& views);
};
```
...
```cpp
// SQLCC FailureRecovery实现
// 源码位置: /home/liying/sqlcc/src/distributed/failure_recovery.h

class FailureRecovery {
public:
    // 节点故障恢复
    bool recoverNodeFailure(NodeID failedNode) {
        // 1. 标记节点为不可用
        markNodeUnavailable(failedNode);
        
        // 2. 迁移故障节点的数据
        bool migrationSuccess = migrateNodeData(failedNode);
    ...
    // 网络分区恢复
    bool recoverNetworkPartition(
        const std::vector<std::vector<NodeID>>& partitionGroups) {
        
        // 1. 识别主分区（包含多数节点的分区）
        auto primaryPartition = identifyPrimaryPartition(partitionGroups);
        auto minorityPartitions = identifyMinorityPartitions(partitionGroups);
        
        // 2. 暂停少数分区的写入操作
        pauseWrites(minorityPartitions);
        
        // 3. 同步数据到少数分区
        bool syncSuccess = syncDataToMinority(primaryPartition, minorityPartitions);
        ...
        // 4. 恢复少数分区的写入操作
        resumeWrites(minorityPartitions);
        
        // 5. 验证数据一致性
        bool consistencyVerified = verifyDataConsistency();
        
        return consistencyVerified;
    }
    
private:
    bool migrateNodeData(NodeID failedNode);
    void updateRoutingTable(NodeID failedNode);
    void broadcastMetadataUpdate();
    std::vector<NodeID> identifyPrimaryPartition(
        const std::vector<std::vector<NodeID>>& partitionGroups);
    std::vector<std::vector<NodeID>> identifyMinorityPartitions(
        const std::vector<std::vector<NodeID>>& partitionGroups);
    void pauseWrites(const std::vector<std::vector<NodeID>>& partitions);
    void resumeWrites(const std::vector<std::vector<NodeID>>& partitions);
    bool syncDataToMinority(
        const std::vector<NodeID>& primary,
        const std::vector<std::vector<NodeID>>& minorities);
...
┌─────────────────────────────────────────────────────────┐
│                     负载均衡层                           │
│  Nginx (SSL终止、请求路由、健康检查)                     │
└─────────────────────────────────────────────────────────┘
                            │
        ┌───────────────────┼───────────────────┐
        │                   │                   │
┌───────▼──────┐   ┌───────▼──────┐   ┌───────▼──────┐
│  SQLCC节点1  │   │  SQLCC节点2  │   │  SQLCC节点3  │
│  (分片1-3)   │   │  (分片4-6)   │   │  (分片7-9)   │
└──────────────┘   └──────────────┘   └──────────────┘
        │                   │                   │
        └───────────────────┼───────────────────┘
...
```yaml
# sqlcc-distributed.yaml
cluster:
  name: "sqlcc-production"
  region: "us-east-1"
...
通过深入理解SQLCC的分布式架构设计，我们可以看到现代数据库系统如何从单机演进到集群，如何通过分片、复制、分布式事务等技术实现横向扩展和高可用性。这些技术不仅适用于数据库系统，也为构建其他分布式系统提供了宝贵的设计思路。
