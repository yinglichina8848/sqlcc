...
## 1.1 数据处理的起源：古代文明的数据管理实践

### 1.1.1 美索不达米亚泥板记账：最早的结构化数据记录

#### 📜 美索不达米亚楔形文字泥板：人类第一个数据库
...
```
泥板上刻下的记录示例：
URSAG - ŠULGI - MÁ-GUR₈-KID₂
猪   - 舒尔吉 - 大船（船名）

这不仅是简单的记录，更是一种结构化的数据表达：
├── 实体标识 (猪)
├── 所有者信息 (舒尔吉)
└── 位置信息 (船名)
```
...
```mermaid
graph TD
    A[原始需求: 财产管理与记录] --> B[结构化标识: 实体-属性-关系]
    B --> C[分类存储: 按主题组织泥板]
    C --> D[索引检索: 按所有者或位置查找]
    D --> E[一致性保证: 手工交叉验证]
```
...
```mermaid
graph LR
A[原始: 泥板刻痕] --> B[中世纪: 账本记录] --> C[工业化: 穿孔卡片]
