...
| 模板 | 路径 | 用途 |
|------|------|------|
| 需求模板 | `docs/sdd/templates/requirements_template.md` | EARS 格式需求定义 |
| 设计模板 | `docs/sdd/templates/design_template.md` | 架构设计、接口定义 |
| 任务模板 | `docs/sdd/templates/tasks_template.md` | 任务分解、进度跟踪 |
| **确认模板** | `docs/sdd/templates/verification_template.md` | **测试验证、版本发布** |
...
│  ┌─────────────┐    ┌─────────────┐    ┌─────────────┐                 │
│  │   spec-     │───▶│   spec-     │───▶│   spec-     │                 │
│  │   tasks     │    │   impl      │    │ verification│                 │
│  │  任务分解    │    │  代码实现    │    │  **验证确认** │                 │
│  └─────────────┘    └─────────────┘    └─────────────┘                 │
│        │                   │                   │                        │
│        ▼                   ▼                   ▼                        │
│   任务依赖图           代码/测试           编译测试验证                   │
│   燃尽图跟踪           BUILD 配置          版本发布确认                   │
│                                               │                        │
│                                               ▼                        │
│                                        ┌─────────────┐                 │
│                                        │   release   │                 │
...
#### 验证阶段 (spec-verification) 关键检查项

| 检查项 | 说明 | 达标标准 |
|--------|------|---------|
| 编译验证 | Bazel 编译无错误 | 0 错误，警告可接受 |
| 测试验证 | GTest 全部通过 | 100% 通过率 |
| 覆盖率 | 代码覆盖率 | >= 90% (Level 1+), >= 70% (Level 2+) |
| 文档完整 | SDD 文档齐全 | requirements.md, design.md, tasks.md, verification.md |
| 版本升级 | 版本号更新 | 第3位版本号+1 |
| 分支状态 | 代码冻结 | 可选：冻结待发布分支 |

### 5. 确认验证模板 (verification_template.md) 内容

**用途**: 在代码实现完成后，验证是否达到可提交/发布状态
...
SQLCC 采用语义化版本 (SemVer)：

| 版本类型 | 格式 | 说明 | 示例 |
|---------|------|------|------|
| **主版本** | `vX.0.0` | 不兼容的 API 变更 | v2.0.0 |
| **次版本** | `v1.X.0` | 向下兼容的新功能 | v1.5.0 |
| **补丁版本** | `v1.0.X` | 向下兼容的 bug 修复 | v1.0.3 |

**版本升级规则**:
- **patch** (第3位): bug 修复、文档更新、测试补充 → `v1.0.0` → `v1.0.1`
- **minor** (第2位): 新功能、向后兼容的改进 → `v1.0.0` → `v1.1.0`
- **major** (第1位): 不兼容的 API 变更 → `v1.0.0` → `v2.0.0`
...
|---------|------|----------|----------|
| `main` | ✅ 活跃 | - | - |
| `release/vX.X` | 🔒 冻结 | 发布候选版本 | 发布完成后 |
| `feature/xxx` | 🔒 冻结 | PR 合并前 | 代码评审通过后 |
