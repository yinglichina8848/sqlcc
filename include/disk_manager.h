...
public:
   ...
   */
  void SetSimulateReadFailure(bool simulate) {
    simulate_read_failure_ = simulate;
  }
   ...
   */
  void SetSimulateWriteFailure(bool simulate) {
    simulate_write_failure_ = simulate;
  }
   ...
   */
  void SetSimulateSeekFailure(bool simulate) {
    simulate_seek_failure_ = simulate;
  }
   ...
   */
  void SetSimulateFlushFailure(bool simulate) {
    simulate_flush_failure_ = simulate;
  }
...
private:
  ...
  // 递归定时互斥锁，保护文件访问
  // Why:
  // 磁盘管理器可能被多个线程同时访问，需要同步机制保证数据一致性，并支持超时避免死锁
  // What:
  // io_mutex_是递归定时互斥锁，保护所有文件I/O操作，支持同一线程多次锁定和超时功能
  // How:
  // 在进行文件I/O操作前加锁，操作完成后解锁，支持递归锁定和try_lock_for超时功能
