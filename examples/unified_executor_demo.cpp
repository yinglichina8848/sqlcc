...
int main() {
  ...
  try {
    ...
    auto unified_executor =
        std::make_shared<sqlcc::UnifiedExecutor>(db_manager);
    ...
    for (const auto &sql : insert_sqls) {
      ...
      if (!statements.empty()) {
        auto result = unified_executor->execute(std::move(statements[0]));
        std::cout << "INSERT: " << (result.success ? "成功" : "失败")
                  << std::endl;
      }
    }
    ...
    // UPDATE语句（演示索引优化）
    {
      ...
      if (!statements.empty()) {
        ...
        std::cout << "  是否使用索引: " << (context.used_index ? "是" : "否")
                  << std::endl;
    ...
    std::cout << "新的架构为复杂查询、高级JOIN、子查询和查询优化器奠定了基础。"
              << std::endl;
