...
FunctionDefinition::FunctionDefinition(const std::string &name)
    : name_(name),
      ...
      hasReturnDataType_(false),
      hasLanguage_(false),
      hasDeterministic_(false) {}
...
FunctionDefinition::ReturnType FunctionDefinition::getReturnType() const {
    return returnType_;
}

std::string FunctionDefinition::getReturnTypeString() const {
    switch (returnType_) {
...
void FunctionDefinition::addParameter(const ProcedureParameter &param) {
    parameters_.push_back(param);
}

const std::vector<ProcedureParameter> &FunctionDefinition::getParameters() const {
    return parameters_;
}

void FunctionDefinition::setReturnDataType(const std::string &dataType) {
    returnDataType_ = dataType;
    hasReturnDataType_ = true;
}

const std::string &FunctionDefinition::getReturnDataType() const {
    return returnDataType_;
}

bool FunctionDefinition::hasReturnDataType() const {
    return hasReturnDataType_;
}
...
const std::string &FunctionDefinition::getLanguage() const {
    return language_;
}

bool FunctionDefinition::hasLanguage() const {
    return hasLanguage_;
}
...
const std::string &FunctionDefinition::getBody() const {
    return body_;
}

void FunctionDefinition::setDeterministic(bool deterministic) {
    deterministic_ = deterministic;
    hasDeterministic_ = true;
}
...
bool FunctionDefinition::hasDeterministic() const {
    return hasDeterministic_;
}

// ==================== Create Function ====================

CreateFunctionStatement::CreateFunctionStatement(std::unique_ptr<FunctionDefinition> functionDef)
    : Statement(Type::CREATE_FUNCTION), functionDef_(std::move(functionDef)) {}

CreateFunctionStatement::~CreateFunctionStatement() {}

const FunctionDefinition &CreateFunctionStatement::getFunctionDefinition() const {
    return *functionDef_;
}

std::unique_ptr<FunctionDefinition> CreateFunctionStatement::takeFunctionDefinition() {
    return std::move(functionDef_);
}

void CreateFunctionStatement::accept(NodeVisitor &visitor) {
    // Default implementation
}

// ==================== Drop Function ====================

DropFunctionStatement::DropFunctionStatement(const std::string &functionName)
    : Statement(Type::DROP_FUNCTION), functionName_(functionName),
      ifExists_(false) {}

DropFunctionStatement::~DropFunctionStatement() {}

const std::string &DropFunctionStatement::getFunctionName() const {
    return functionName_;
}

bool DropFunctionStatement::isIfExists() const {
    return ifExists_;
}

void DropFunctionStatement::setIfExists(bool ifExists) {
    ifExists_ = ifExists;
}

void DropFunctionStatement::accept(NodeVisitor &visitor) {
    // Default implementation
}
