...
Lexer::Lexer(const std::string &input)
    : input_(input), position_(0), line_(1), column_(1),
      currentToken_(Token::END_OF_INPUT, "", 1, 1), hasCachedToken_(false) {
  // 初始化关键字映射表
}
...
Lexer::Position Lexer::getPosition() const { return {line_, column_}; }
...
bool Lexer::isIdentifierStart(char c) const { return isLetter(c) || c == '_'; }
...
bool Lexer::isLetter(char c) const { return std::isalpha(c); }
...
bool Lexer::isEOF() const { return position_ >= input_.size(); }
...
Token::Type Lexer::getKeywordType(const std::string &identifier) const {
  ...
  static const std::unordered_map<std::string, Token::Type> keywords = {
      ...
      {"TRANSACTION", Token::KEYWORD_TRANSACTION},
      {"START", Token::KEYWORD_START},
      {"COMMIT", Token::KEYWORD_COMMIT},
      {"ROLLBACK", Token::KEYWORD_ROLLBACK},
      {"SAVEPOINT", Token::KEYWORD_SAVEPOINT},
      {"AUTOCOMMIT", Token::KEYWORD_AUTOCOMMIT},
      {"ISOLATION", Token::KEYWORD_ISOLATION},
      {"LEVEL", Token::KEYWORD_LEVEL},
      {"READ", Token::KEYWORD_READ},
      {"WRITE", Token::KEYWORD_WRITE},
      {"UNCOMMITTED", Token::KEYWORD_UNCOMMITTED},
      {"COMMITTED", Token::KEYWORD_COMMITTED},
      {"REPEATABLE", Token::KEYWORD_REPEATABLE},
      {"SERIALIZABLE", Token::KEYWORD_SERIALIZABLE}};
...
Token Lexer::scanNextToken() {
  ...
  case '%':
    advance();
    column_++;
    return Token(Token::OPERATOR_MODULO, "%", line_, currentColumn);
  case '(':
    advance();
    column_++;
  ...
  case ':':
    ...
    if (match(':')) {
      advance();
      column_++;
      return Token(Token::PUNCTUATION_DOUBLE_COLON, "::", line_, currentColumn);
    }
    return Token(Token::PUNCTUATION_COLON, ":", line_, currentColumn);
  default:
    // 无效字符
    advance();
    column_++;
