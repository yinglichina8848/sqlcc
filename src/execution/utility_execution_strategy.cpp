...
ExecutionResult UtilityExecutionStrategy::executeUse(const sql_parser::UseStatement& stmt,
    ...
    if (auto db_manager = context.getDatabaseManager()) {
        // 尝试切换到指定的数据库
        if (db_manager->selectDatabase(stmt.database_name)) {
            // 更新执行上下文中的当前数据库
            context.setCurrentDatabase(stmt.database_name);
            return createSuccessResult("Database changed to " + stmt.database_name);
        } else {
            return createErrorResult("Database '" + stmt.database_name + "' does not exist");
        }
    }
...
ExecutionResult UtilityExecutionStrategy::executeShow(const sql_parser::ShowStatement& stmt,
    ...
    if (auto db_manager = context.getDatabaseManager()) {
        ...
        if (stmt.show_type == sql_parser::ShowStatement::Type::DATABASES) {
            // 显示所有数据库
            auto databases = db_manager->getAllDatabases();
            result_stream << "Databases:\n";
            for (const auto& db : databases) {
                result_stream << "- " << db << "\n";
            }
        } else if (stmt.show_type == sql_parser::ShowStatement::Type::TABLES) {
            // 显示当前数据库中的所有表
            auto current_db = context.getCurrentDatabase();
            if (!current_db.empty()) {
                auto tables = db_manager->getTableNames(current_db);
        ...
        } else if (stmt.show_type == sql_parser::ShowStatement::Type::COLUMNS) {
            // 显示指定表的列信息
            auto current_db = context.getCurrentDatabase();
            if (!current_db.empty() && !stmt.table_name.empty()) {
                ...
                if (!table_schema.empty()) {
                    result_stream << "Columns in " << current_db << "." << stmt.table_name << ":\n";
