...
SetOperationExecutor::SetOperationExecutor(std::shared_ptr<SqlExecutor> sql_executor)
    : sql_executor_(sql_executor), memory_limit_(1024 * 1024 * 1024) { // 默认1GB内存限制
}
...
ExecutionResult SetOperationExecutor::execute(const SetOperationNode& operation) {
    ...
    try {
        ...
        if (!operation.is_valid()) {
            throw InvalidOperationException("Invalid set operation: operation is not valid");
        }
        ...
        stats_.left_execution_time = std::chrono::duration_cast<std::chrono::milliseconds>(left_end - left_start);
        ...
        stats_.right_execution_time = std::chrono::duration_cast<std::chrono::milliseconds>(right_end - right_start);
        
        // 5. 验证结果集兼容性
        if (!validate_result_compatibility(left_result, right_result)) {
            throw IncompatibleResultException("Incompatible result sets for set operation");
        }
        ...
        stats_.operation_execution_time = std::chrono::duration_cast<std::chrono::milliseconds>(operation_end - operation_start);
        
        // 7. 更新统计信息
        auto end_time = std::chrono::steady_clock::now();
        stats_.total_execution_time = std::chrono::duration_cast<std::chrono::milliseconds>(end_time - start_time);
...
ExecutionStats SetOperationExecutor::get_stats() const {
    return stats_;
}

ExecutionResult SetOperationExecutor::execute_union(const SetOperationNode& operation, 
...
ExecutionResult SetOperationExecutor::execute_intersect(const SetOperationNode& operation, 
                                                       const ExecutionResult& left, 
                                                       const ExecutionResult& right) {
    return ResultSetCombiner::intersect(left, right, operation.is_all());
}

ExecutionResult SetOperationExecutor::execute_except(const SetOperationNode& operation, 
                                                    const ExecutionResult& left, 
                                                    const ExecutionResult& right) {
    return ResultSetCombiner::except(left, right, operation.is_all());
}

ExecutionResult SetOperationExecutor::execute_subquery(const std::unique_ptr<SelectStatement>& subquery) {
    if (!subquery) {
        throw InvalidOperationException("Null subquery in set operation");
    }
    
    // 使用SQL执行器执行子查询
    return sql_executor_->execute_select(*subquery);
}

bool SetOperationExecutor::validate_result_compatibility(const ExecutionResult& left, 
                                                        const ExecutionResult& right) {
    // 检查列数是否相同
...
ExecutionResult ResultSetCombiner::union_distinct(const ExecutionResult& left, 
                                                 const ExecutionResult& right) {
    ExecutionResult result;
    result.column_metadata = left.column_metadata;
...
RowKey ResultSetCombiner::generate_row_key(const Row& row, 
