...
int main(int argc, char* argv[]) {
    ...
    // 解析命令行参数
    int opt;
    while ((opt = getopt(argc, argv, "h:p:u:P:e")) != -1) {
        switch (opt) {
            case 'h':
                host = optarg;
                break;
            ...
            case 'e':
                ...
                break;
            default:
                std::cerr << "Usage: " << argv[0] << " [-h host] [-p port] [-u username] [-P password] [-e]" << std::endl;
                std::cerr << "  -e: Enable AES-256 encryption" << std::endl;
                return 1;
        }
    }
