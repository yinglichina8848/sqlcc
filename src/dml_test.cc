...
int main() {
    try {
        ...
        // 测试创建表（为DML操作准备）
        std::cout << "\n=== 创建表测试 ===" << std::endl;
        std::string create_result = executor.Execute("CREATE TABLE users (id INT PRIMARY KEY, name VARCHAR(50), age INT)");
        std::cout << create_result << std::endl;
        
        // 测试插入数据
        std::cout << "\n=== 插入数据测试 ===" << std::endl;
        std::string insert_result1 = executor.Execute("INSERT INTO users VALUES (1, 'Alice', 25)");
        std::cout << insert_result1 << std::endl;
        
        std::string insert_result2 = executor.Execute("INSERT INTO users VALUES (2, 'Bob', 30)");
        std::cout << insert_result2 << std::endl;
        
        std::string insert_result3 = executor.Execute("INSERT INTO users VALUES (3, 'Charlie', 35)");
        ...
        std::string select_result2 = executor.Execute("SELECT name, age FROM users");
        std::cout << select_result2 << std::endl;
        
        // 测试更新数据
        std::cout << "\n=== 更新数据测试 ===" << std::endl;
        std::string update_result = executor.Execute("UPDATE users SET age = 26 WHERE id = 1");
        std::cout << update_result << std::endl;
        
        // 验证更新结果
        std::string select_result3 = executor.Execute("SELECT * FROM users WHERE id = 1");
        std::cout << select_result3 << std::endl;
        
        // 测试删除数据
        std::cout << "\n=== 删除数据测试 ===" << std::endl;
        std::string delete_result = executor.Execute("DELETE FROM users WHERE id = 2");
