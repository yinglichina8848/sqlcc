...
MySQLProtocolHandler::MySQLProtocolHandler(int client_fd)
...
void MySQLProtocolHandler::send_handshake() {
  ...
  write(client_fd_, &version, sizeof(version));

  // 发送服务器版本字符串（以\0结尾）
  write(client_fd_, handshake_.server_version.c_str(),
        handshake_.server_version.size() + 1);

  // 发送线程ID
  write(client_fd_, &handshake_.thread_id, sizeof(handshake_.thread_id));

  // 发送salt（前8字节）
  write(client_fd_, handshake_.scramble_buf, 8);

  // 填充字节
  uint8_t filler = 0;
  write(client_fd_, &filler, 1);

  // 发送能力标志低16位
  uint16_t cap_low =
      static_cast<uint16_t>(handshake_.server_capabilities & 0xFFFF);
  write(client_fd_, &cap_low, sizeof(cap_low));

  // 发送字符集
  write(client_fd_, &handshake_.server_default_collation, 1);

  // 发送状态标志
  write(client_fd_, &handshake_.server_status,
        sizeof(handshake_.server_status));

  // 发送能力标志高16位
  uint16_t cap_high =
      static_cast<uint16_t>((handshake_.server_capabilities >> 16) & 0xFFFF);
  write(client_fd_, &cap_high, sizeof(cap_high));

  // 填充剩余字段
  uint8_t auth_plugin_len = 20;
  write(client_fd_, &auth_plugin_len, 1);

  // 填充保留字段
  uint8_t reserved[10] = {0};
  write(client_fd_, reserved, 10);

  // 发送salt剩余12字节
  write(client_fd_, handshake_.scramble_buf + 8, 12);

  // 发送认证插件名
  write(client_fd_, handshake_.auth_plugin_name, 20);
}

bool MySQLProtocolHandler::handle_client_response() {
  // 读取客户端响应（至少1字节命令）
  uint8_t cmd;
  ssize_t ret = read(client_fd_, &cmd, 1);
  if (ret <= 0)
    return false;
