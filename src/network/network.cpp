...
bool SessionManager::Authenticate(int session_id, const std::string &user,
...
bool SessionManager::CheckPermission(int session_id,
...
bool ClientConnection::SendData(const std::vector<char> &data) {
  ...
  while (total_sent < static_cast<ssize_t>(data_size)) {
    ssize_t sent =
        send(fd_, data.data() + total_sent, data_size - total_sent, 0);
    if (sent < 0) {
...
std::vector<char> ClientConnection::ReceiveData() {
  ...
  while (header_received < static_cast<ssize_t>(sizeof(MessageHeader))) {
    ssize_t received = recv(fd_, header_ptr + header_received,
                            sizeof(MessageHeader) - header_received, 0);
    if (received < 0) {
      if (errno == EAGAIN || errno == EWOULDBLOCK) {
        // 非阻塞模式下需要重试
        std::this_thread::sleep_for(std::chrono::milliseconds(1));
  ...
  while (body_received < static_cast<ssize_t>(header.length)) {
    ssize_t received = recv(fd_, body.data() + body_received,
                            header.length - body_received, 0);
    if (received < 0) {
      if (errno == EAGAIN || errno == EWOULDBLOCK) {
        // 非阻塞模式下需要重试
        std::this_thread::sleep_for(std::chrono::milliseconds(1));
  ...
  std::memcpy(message.data() + sizeof(MessageHeader), body.data(),
              header.length);
...
ConnectionHandler::ConnectionHandler(
    int fd, std::shared_ptr<SessionManager> session_manager)
    : fd_(fd), closed_(false), session_manager_(session_manager), recv_pos_(0),
      send_pos_(0) {
  // 设置socket为非阻塞模式
...
bool ConnectionHandler::ReadData() {
  ...
  ssize_t bytes_received =
      recv(fd_, recv_buffer_.data() + current_size, 1024, 0);
...
bool ConnectionHandler::WriteData() {
  ...
  ssize_t bytes_sent = send(fd_, send_buffer_.data() + send_pos_,
                            send_buffer_.size() - send_pos_, 0);
...
std::vector<char>
MessageProcessor::ProcessConnectMessage(const std::vector<char> &message) {
  // 处理连接消息的实现
...
std::vector<char>
MessageProcessor::ProcessAuthMessage(const std::vector<char> &message) {
  // 处理认证消息的实现
  std::vector<char> response;
  return response;
}
...
std::vector<char>
MessageProcessor::ProcessQueryMessage(const std::vector<char> &message) {
  // 处理查询消息的实现
  std::vector<char> response;
  return response;
}
...
std::vector<char>
MessageProcessor::ProcessCloseMessage(const std::vector<char> &message) {
  // 处理关闭消息的实现
  std::vector<char> response;
  return response;
}
...
std::vector<char>
MessageProcessor::BuildResponse(uint16_t type, const std::vector<char> &data) {
  // 构建响应消息的实现
  std::vector<char> response;
  return response;
}
...
ServerNetworkManager::ServerNetworkManager(int port, int max_connections)
    : listen_fd_(-1), epoll_fd_(-1), port_(port),
      max_connections_(max_connections) {}

ServerNetworkManager::~ServerNetworkManager() { Stop(); }
...
void ServerNetworkManager::AcceptConnection() {
  ...
  auto connection_handler =
      std::make_shared<ConnectionHandler>(client_fd, session_manager_);
