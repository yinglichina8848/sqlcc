...
void signalHandler(int signal) {
    if (signal == SIGINT) {
        std::cout << "\nInterrupted by user. Type 'exit' or 'quit' to exit." << std::endl;
        // 重置readline的输入行（如果可用）
#ifdef USE_READLINE
        rl_replace_line("", 0);
        rl_on_new_line();
...
void showHelp() {
    ...
    std::cout << "  describe <table>, .desc <table>  Show table structure" << std::endl;
    std::cout << "  show create table <table>  Show CREATE TABLE statement" << std::endl;
    std::cout << "  .exit, .quit  Exit the program" << std::endl;
    std::cout << std::endl;
 ...
 */
void showVersion() {
    std::cout << "isql (SqlCC) " << SQLCC_VERSION << std::endl;
...
bool handleInteractiveCommand(const std::string& cmd, SqlExecutor& executor) {
    ...
    if (cmd == ".schema" || cmd == "show tables" || cmd == ".show tables") {
        std::string result = executor.ListTables();
        std::cout << result << std::endl;
        return false;
    }
    
    // 处理DESCRIBE TABLE命令
    if (cmd.substr(0, 8) == "describe" || cmd.substr(0, 4) == ".desc" || cmd.substr(0, 7) == "show create table") {
        std::string table_name;
        size_t pos = cmd.find_last_of(' ');
        if (pos != std::string::npos) {
            table_name = cmd.substr(pos + 1);
            // 移除可能的引号
            if (!table_name.empty() && (table_name[0] == '\'' || table_name[0] == '"')) {
                table_name = table_name.substr(1, table_name.length() - 2);
            }
            std::string result = executor.ShowTableSchema(table_name);
            std::cout << result << std::endl;
        } else {
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        // 显示欢迎信息
        std::cout << "Welcome to isql (SqlCC " << SQLCC_VERSION << ")" << std::endl;
        ...
        while (g_running) {
            ...
            if (!input.empty() && (input[0] == '.' || input == "exit" || input == "quit" || input == "help")) {
