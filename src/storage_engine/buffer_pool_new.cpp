...
BufferPool::BufferPool(DiskManager *disk_manager, size_t pool_size,
    ...
    : disk_manager_(disk_manager), config_manager_(config_manager),
      pool_size_(pool_size), lock_timeout_ms_(config_manager.GetInt(
                                 "buffer_pool.lock_timeout_ms", 3000)) {

  SQLCC_LOG_INFO("Initializing simplified BufferPool with size: " +
...
Page *BufferPool::FetchPage(int32_t page_id) {
  ...
  SQLCC_LOG_DEBUG("Page " + std::to_string(page_id) +
                  " not in buffer, loading from disk");
...
bool BufferPool::FlushPage(int32_t page_id) {
  ...
  if (lock.try_lock_for(std::chrono::milliseconds(lock_timeout_ms_))) {
    ...
    SQLCC_LOG_WARN("Failed to reacquire lock after flushing page " +
                   std::to_string(page_id));
  }
...
bool BufferPool::DeletePage(int32_t page_id) {
  ...
  if (dirty_it != dirty_pages_.end() && dirty_it->second) {
    ...
    if (!flush_success) {
      SQLCC_LOG_ERROR("Failed to flush dirty page " + std::to_string(page_id) +
                      " before deletion");
      return false;
    }
  }
  ...
  if (!disk_manager_->DeallocatePage(page_id)) {
    SQLCC_LOG_WARN("Failed to notify disk manager to deallocate page " +
                   std::to_string(page_id));
...
bool BufferPool::Resize(size_t new_pool_size) {
  ...
  SQLCC_LOG_INFO("Resizing buffer pool from " + std::to_string(pool_size_) +
  ...
  if (new_pool_size < page_table_.size()) {
    ...
    for (size_t i = 0; i < pages_to_evict; ++i) {
      ...
      if (victim_id == -1) {
        SQLCC_LOG_ERROR(
            "Cannot resize buffer pool - no pages available for eviction");
        return false;
      }
  ...
  SQLCC_LOG_INFO("Buffer pool resized successfully to " +
                 std::to_string(pool_size_));
  return true;
}
