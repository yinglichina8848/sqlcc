...
#include "include/utils/file_descriptor.h"
#include <algorithm>
#include <cstring>
#include <fcntl.h>
#include <filesystem>
...
// What:
// 构造函数接收数据库文件路径和配置管理器引用，打开文件流，初始化文件大小和页面计数器
// How:
// 使用fstream打开文件，如果文件不存在则创建新文件，获取文件大小并计算页面数量
DiskManager::DiskManager(const std::string &db_file,
                         ConfigManager &config_manager)
    : db_file_name_(db_file), config_manager_(config_manager), file_size_(0),
      next_page_id_(0) {
  // 从配置管理器获取锁超时时间，与BufferPool保持一致
  lock_timeout_ms_ =
      config_manager_.GetInt("disk_manager.lock_timeout_ms", 3000);
  ...
  SQLCC_LOG_INFO(
      "Initializing DiskManager for database file: " + db_file_name_ +
      " with lock timeout: " + std::to_string(lock_timeout_ms_) + "ms");
  ...
  if (db_io_.good()) {
    ...
    SQLCC_LOG_INFO("Opened database file: " + db_file_name_ +
                   ", file size: " + std::to_string(file_size_) +
                   ", next page ID: " + std::to_string(next_page_id_));
  } else {
    // 文件打开失败，抛出异常
...
// How:
// 计算页面在文件中的偏移量，使用seekp定位到该位置，然后调用write方法写入页面数据
bool DiskManager::WritePage(int32_t page_id, const char *page_data) {
  ...
  if (!lock.try_lock_for(std::chrono::milliseconds(lock_timeout_ms_))) {
    SQLCC_LOG_WARN("Failed to acquire disk manager lock within timeout: " +
                   std::to_string(lock_timeout_ms_) + "ms");
    return false;
  }
  ...
  if (page_data == nullptr) {
    std::string error_msg =
        "Null page data pointer for page ID: " + std::to_string(page_id);
    SQLCC_LOG_ERROR(error_msg);
    return false;
  }
  ...
  SQLCC_LOG_DEBUG("Writing page ID " + std::to_string(page_id) + " at offset " +
  ...
  if (db_io_.fail()) {
    std::string error_msg =
        "Failed to flush page " + std::to_string(page_id) + " to disk";
    SQLCC_LOG_ERROR(error_msg);
    db_io_.clear();
    return false;
  }
...
// What:
// ReadPage方法接收页面ID和页面数据缓冲区指针，从磁盘文件中读取对应页面的数据
// How:
// 计算页面在文件中的偏移量，使用seekg定位到该位置，然后调用read方法读取页面数据
bool DiskManager::ReadPage(int32_t page_id, char *page_data) {
  ...
  if (it != free_pages_.end()) {
    SQLCC_LOG_DEBUG("Page ID " + std::to_string(page_id) +
                    " has been deallocated and cannot be read");
    return false;
  }
  ...
  // How: 使用SQLCC_LOG_DEBUG宏记录调试级别日志
  SQLCC_LOG_DEBUG("Reading page ID " + std::to_string(page_id) + " at offset " +
                  std::to_string(offset));
  ...
  if (offset >= file_size_) {
    std::string warn_msg =
        "Page " + std::to_string(page_id) + " does not exist in file";
...
int32_t DiskManager::AllocatePage() {
  ...
  SQLCC_LOG_DEBUG("Allocated new page ID: " + std::to_string(page_id) +
                  ", next available: " + std::to_string(next_page_id_));
...
bool DiskManager::DeallocatePage(int32_t page_id) {
  ...
  if (page_id < 0) {
    SQLCC_LOG_WARN("Attempted to deallocate invalid page ID: " +
...
bool DiskManager::BatchReadPages(const std::vector<int32_t> &page_ids,
  ...
  if (page_ids.size() != data_buffers.size() || page_ids.empty()) {
    SQLCC_LOG_ERROR(
        "Invalid batch read parameters: mismatched sizes or empty arrays");
    return false;
  }
...
// What:
// OnConfigChange方法处理配置变更事件，根据变更的配置项调整相应的磁盘管理器参数
// How: 根据配置键判断变更类型，执行相应的调整操作
void DiskManager::OnConfigChange(const std::string &key,
