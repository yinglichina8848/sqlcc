 ...
 * What:
 * 实现ConfigManager类的所有成员函数，提供配置文件的加载、解析、保存和访问功能
 ...
 * How:
 * 使用互斥锁保护配置加载过程，先加载默认配置，再加载主配置文件，最后加载环境特定配置
 */
bool ConfigManager::LoadConfig(const std::string &config_file_path,
  ...
  if (!ParseConfigFile(config_file_path)) {
    std::cerr << "Failed to load main config file: " << config_file_path
              << std::endl;
    return false;
  }
  ...
  if (!env.empty()) {
    ...
    if (std::filesystem::exists(env_config_file)) {
      if (!ParseConfigFile(env_config_file)) {
        std::cerr << "Failed to load environment config file: "
                  << env_config_file << std::endl;
        return false;
      }
    }
  }
 ...
 * How:
 * 使用互斥锁保护配置访问，查找配置键并尝试转换为布尔值，返回对应的值或默认值
 */
bool ConfigManager::GetBool(const std::string &key, bool default_value) const {
  ...
  if (it != config_map_.end()) {
    // 尝试将ConfigValue转换为布尔值
    // Why: 需要将存储的ConfigValue转换为布尔类型
    // What: 使用std::visit访问ConfigValue，将其转换为布尔值
    // How: 使用lambda函数处理不同类型的值
    return std::visit(
        [&default_value](const auto &v) -> bool {
 ...
 * What: GetInt方法从配置中获取指定键的int值，支持从double类型转换
 * How: 使用互斥锁保护配置访问，查找配置键并尝试转换为整数，返回对应的值或默认值
 */
int ConfigManager::GetInt(const std::string &key, int default_value) const {
  ...
  if (it != config_map_.end()) {
    // 尝试将ConfigValue转换为整数
    // Why: 需要将存储的ConfigValue转换为整数类型
    // What: 使用std::visit访问ConfigValue，将其转换为整数
    // How: 使用lambda函数处理不同类型的值
    return std::visit(
        [&default_value](const auto &v) -> int {
          if constexpr (std::is_same_v<std::decay_t<decltype(v)>, bool>) {
            return v ? 1 : 0;
          } else if constexpr (std::is_same_v<std::decay_t<decltype(v)>, int>) {
            return v;
            ...
            return static_cast<int>(v);
                                              ...
                                              std::string>) {
            // 尝试将字符串值转换为整数
            // Why: 需要将存储的字符串值转换为整数类型
            // What: 使用std::stoi尝试转换字符串为整数
            // How: 调用std::stoi并捕获可能的异常
            try {
              return std::stoi(v);
            } catch (const std::exception &) {
              // 转换失败，尝试转换为double再转为int
              try {
                return static_cast<int>(std::stod(v));
              } catch (const std::exception &) {
                // 仍然失败，返回默认值
                return default_value;
              }
 ...
 * How:
 * 使用互斥锁保护配置访问，查找配置键并尝试转换为double，返回对应的值或默认值
 */
double ConfigManager::GetDouble(const std::string &key,
  ...
  auto it = config_map_.find(key);
  if (it != config_map_.end()) {
    // 尝试将ConfigValue转换为double
    // Why: 需要将存储的ConfigValue转换为double类型
    // What: 使用std::visit访问ConfigValue，将其转换为double
    // How: 使用lambda函数处理不同类型的值
    return std::visit(
        [&default_value](const auto &v) -> double {
          if constexpr (std::is_same_v<std::decay_t<decltype(v)>, bool>) {
            return v ? 1.0 : 0.0;
          } else if constexpr (std::is_same_v<std::decay_t<decltype(v)>, int>) {
            ...
            return v;
                                              ...
                                              std::string>) {
            // 尝试将字符串值转换为double
            // Why: 需要将存储的字符串值转换为double类型
            // What: 使用std::stod尝试转换字符串为double
            // How: 调用std::stod并捕获可能的异常
            try {
              return std::stod(v);
            } catch (const std::exception &) {
              // 转换失败，尝试转换为int再转为double
              try {
                return static_cast<double>(std::stoi(v));
              } catch (const std::exception &) {
                // 仍然失败，返回默认值
                return default_value;
              }
...
std::string ConfigManager::GetString(const std::string &key,
  ...
  if (it != config_map_.end()) {
    // 如果找到键，则返回对应的值
    // Why: 需要返回找到的配置值
    // What: 使用std::visit访问ConfigValue，将其转换为字符串
    // How: 使用lambda函数处理不同类型的值
    return std::visit(
...
bool ConfigManager::SaveToFile(const std::string &file_path) const {
  ...
  for (const auto &[key, value] : config_map_) {
    ...
    if (dot_pos != std::string::npos) {
      ...
      // 处理ConfigValue类型，将其转换为字符串
      // Why: config_map_存储的是ConfigValue类型的值，需要转换为字符串
      // What: 使用std::visit访问ConfigValue，将其转换为字符串
      // How: 使用lambda函数处理不同类型的值
      std::visit(
          [&file](const auto &v) {
...
ConfigManager::GetKeysWithPrefix(const std::string &prefix) const {
  std::lock_guard<std::mutex> lock(config_mutex_);
...
bool ConfigManager::ParseConfigLine(const std::string &line,
  ...
  if (equal_pos != std::string::npos) {
    ...
    if (value == "true" || value == "false") {
      config_map_[full_key] = value; // 直接存储字符串表示
    } else if (value.find('.') != std::string::npos) {
      try {
        std::stod(value);              // 验证是否为有效的浮点数
        config_map_[full_key] = value; // 存储字符串表示
      } catch (const std::exception &) {
        config_map_[full_key] = value; // 不是浮点数，作为字符串存储
      }
    } else {
      try {
        std::stoi(value);              // 验证是否为有效的整数
        config_map_[full_key] = value; // 存储字符串表示
      } catch (const std::exception &) {
        config_map_[full_key] = value; // 不是整数，作为字符串存储
      }
    }
  }
...
void ConfigManager::LoadDefaultConfig() {
  ...
  config_map_["database.db_file_size_limit"] = std::to_string(1024);
  config_map_["database.page_size"] = std::to_string(8192);

  // 缓冲池配置
  // Why: 需要设置缓冲池相关的默认配置
  // What: 设置缓冲池大小、替换策略、预取策略等配置
  // How: 直接在config_map_中设置键值对
  config_map_["buffer_pool.pool_size"] = std::to_string(64);
  config_map_["buffer_pool.replacement_policy"] = std::string("LRU");
  config_map_["buffer_pool.prefetch_strategy"] = std::string("SEQUENTIAL");
  config_map_["buffer_pool.prefetch_window"] = std::to_string(4);
  config_map_["buffer_pool.flush_interval"] = std::to_string(30);
  config_map_["buffer_pool.dirty_page_threshold"] = std::to_string(0.75);

  // 磁盘管理器配置
  // Why: 需要设置磁盘管理器相关的默认配置
  // What: 设置IO线程池大小、批量读写大小、异步IO等配置
  // How: 直接在config_map_中设置键值对
  config_map_["disk_manager.io_thread_pool_size"] = std::to_string(4);
  config_map_["disk_manager.batch_read_size"] = std::to_string(8);
  config_map_["disk_manager.batch_write_size"] = std::to_string(8);
  ...
  // How: 直接在config_map_中设置键值对
  config_map_["storage_engine.concurrency_control"] =
      std::string("PESSIMISTIC");
  config_map_["storage_engine.lock_timeout"] = std::to_string(5000);
  config_map_["storage_engine.deadlock_detection_interval"] =
      std::to_string(1000);
  config_map_["storage_engine.isolation_level"] = std::string("READ_COMMITTED");
  config_map_["storage_engine.checkpoint_interval"] = std::to_string(60);
  ...
  config_map_["logging.log_file_size_limit"] = std::to_string(100);
  config_map_["logging.log_file_backup_count"] = std::to_string(5);
  ...
  config_map_["performance.stats_interval"] = std::to_string(10);
