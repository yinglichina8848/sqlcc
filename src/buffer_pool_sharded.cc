...
BufferPoolSharded::BufferPoolSharded(DiskManager* disk_manager, ConfigManager& config_manager,
    ...
    if (num_shards & (num_shards - 1)) {
        ...
        SQLCC_LOG_INFO("Adjusting shard count to power of 2: " + std::to_string(num_shards_));
    } else {
    ...
    SQLCC_LOG_INFO("Sharded BufferPool initialized with " + std::to_string(num_shards_) + 
                  " shards, each with " + std::to_string(shard_size) + " pages");
}
...
Page* BufferPoolSharded::FetchPage(int32_t page_id, bool exclusive) {
    ...
    bool read_success = disk_manager_->ReadPage(page_id, static_cast<char*>(page->GetData()));
    
    if (!read_success) {
        SQLCC_LOG_DEBUG("Failed to read page ID " + std::to_string(page_id));
        return nullptr;
    }
    ...
    if (shard.current_size >= shard.max_size) {
        ...
        if (replaced_page_id == -1) {
            SQLCC_LOG_ERROR("Failed to replace page in shard");
            return nullptr;
        }
    }
...
bool BufferPoolSharded::FlushPage(int32_t page_id) {
    ...
    bool write_success = disk_manager_->WritePage(page_id, static_cast<char*>(page_wrapper->page->GetData()));
    ...
    for (size_t i = 0; i < num_shards_; ++i) {
        ...
        for (const auto& pair : shard.page_table) {
            ...
            if (page_wrapper->is_dirty) {
                // 释放锁，执行磁盘I/O
                lock.~lock_guard();
                disk_manager_->WritePage(page_id, static_cast<char*>(page_wrapper->page->GetData()));
                // 重新获取锁
                std::lock_guard<std::mutex> lock2(shard.mutex);
                page_wrapper->is_dirty = false;
            }
...
std::unordered_map<std::string, double> BufferPoolSharded::GetStats() const {
    ...
    if (stats_.total_accesses > 0) {
        stats["hit_rate"] = static_cast<double>(stats_.total_hits) / stats_.total_accesses;
    } else {
        stats["hit_rate"] = 0.0;
    }
