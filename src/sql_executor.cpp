...
SqlExecutor::SqlExecutor()
    : last_error_(""), execution_stats_(""), current_user_("admin"),
      current_database_("") {
  try {
    // 初始化数据库管理器
    db_manager_ = std::make_shared<DatabaseManager>("./test_db", 8192, 1024, 100);
    ...
    last_error_ =
        std::string("Failed to initialize SqlExecutor: ") + e.what();
    std::cerr << "SqlExecutor initialization error: " << e.what()
              << std::endl;
  }
}
...
SqlExecutor::SqlExecutor(std::shared_ptr<DatabaseManager> db_manager)
    : last_error_(""), execution_stats_(""), current_user_("admin"),
      current_database_(""), db_manager_(db_manager) {
...
bool SqlExecutor::InitializeSystemDatabase() {
  try {
    ...
    SetError(std::string("System database initialization error: ") +
             e.what());
    return false;
  }
}
...
bool SqlExecutor::InitializePermissionValidator() {
  try {
    ...
    permission_validator_ =
        std::make_unique<PermissionValidator>(user_manager_, system_db_);

    return true;
  } catch (const std::exception &e) {
    SetError(std::string("Permission validator initialization error: ") +
             e.what());
    return false;
  }
}
...
// 解析SQL语句
std::unique_ptr<sql_parser::Statement> SqlExecutor::ParseSQL(
    const std::string &sql) {
  try {
    // 创建SQL解析器
    auto parser = sql_parser::ParserNew(sql);

    // 解析SQL语句
    auto statements = parser.parse();

    if (statements.empty()) {
      SetError("SQL parsing failed: no statements returned");
      return nullptr;
    }
...
std::unique_ptr<UnifiedQueryPlan> SqlExecutor::CreateQueryPlan(
  ...
  try {
    ...
    auto query_plan = QueryPlanFactory::createPlan(
        std::move(stmt), db_manager_, user_manager_, system_db_);
