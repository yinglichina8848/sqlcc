...
SqlExecutor::SqlExecutor(DatabaseManager &db_manager) 
    : storage_engine_(nullptr), db_manager_(&db_manager), current_database_("sqlcc") {
  index_executor_ = std::unique_ptr<IndexManager>(nullptr); // TODO: 从StorageEngine获取
  transaction_manager_ = std::unique_ptr<TransactionManager>(nullptr); // TODO: 初始化事务管理器
}

SqlExecutor::SqlExecutor(StorageEngine &storage_engine, DatabaseManager &db_manager)
    : storage_engine_(&storage_engine), db_manager_(&db_manager), current_database_("sqlcc") {
  index_executor_ = std::unique_ptr<IndexManager>(nullptr); // TODO: 从StorageEngine获取index manager
  transaction_manager_ = std::unique_ptr<TransactionManager>(nullptr); // TODO: 初始化事务管理器
}
...
std::string SqlExecutor::ExecuteCreate(const sql_parser::CreateStatement &create_stmt) {
  if (create_stmt.object_type == sql_parser::CreateStatement::ObjectType::DATABASE) {
    return ExecuteCreateDatabase(create_stmt);
  } else if (create_stmt.object_type == sql_parser::CreateStatement::ObjectType::TABLE) {
    return ExecuteCreateTable(create_stmt);
  } else {
    return ExecuteCreateIndex(create_stmt);
  }
}

std::string SqlExecutor::ExecuteDrop(const sql_parser::DropStatement &drop_stmt) {
  if (drop_stmt.object_type == sql_parser::DropStatement::ObjectType::DATABASE) {
    return ExecuteDropDatabase(drop_stmt);
  } else if (drop_stmt.object_type == sql_parser::DropStatement::ObjectType::TABLE) {
    return ExecuteDropTable(drop_stmt);
  } else {
    return ExecuteDropIndex(drop_stmt);
  }
}
...
SqlExecutor::ExecuteStatement(const sql_parser::StatementPtr &stmt) {
  ...
  case sql_parser::Statement::Type::CREATE: {
    const auto& create_stmt = dynamic_cast<const sql_parser::CreateStatement &>(*stmt);
    return ExecuteCreate(create_stmt);
  }

  case sql_parser::Statement::Type::DROP: {
    const auto& drop_stmt = dynamic_cast<const sql_parser::DropStatement &>(*stmt);
    return ExecuteDrop(drop_stmt);
  }
...
std::string SqlExecutor::ExecuteCreateDatabase(const sql_parser::CreateStatement &create_stmt) {
  const std::string &database_name = create_stmt.database_name;
  
  // 检查数据库是否已存在
  if (db_manager_->DatabaseExists(database_name)) {
    SetError("Database '" + database_name + "' already exists");
    return "ERROR: Database '" + database_name + "' already exists\n";
  }
  
  // 创建数据库
  if (!db_manager_->CreateDatabase(database_name)) {
    return "ERROR: Failed to create database\n";
  }
  
  return "Query OK, 1 database created (" + database_name + ")\n";
}

std::string SqlExecutor::ExecuteCreateTable(const sql_parser::CreateStatement &create_stmt) {
  const std::string &table_name = NormalizeTableName(create_stmt.table_name);
  
  // 检查是否有当前数据库
  if (current_database_.empty()) {
    SetError("No database selected. Use 'USE database' to select a database.");
    return "ERROR: No database selected\n";
  }
  
  // 检查表是否已存在
  if (db_manager_->TableExists(current_database_, table_name)) {
  ...
  // 创建表
  if (!db_manager_->CreateTable(current_database_, table_name, create_stmt.columns, create_stmt.constraints)) {
...
std::string SqlExecutor::ExecuteDropDatabase(const sql_parser::DropStatement &drop_stmt) {
  const std::string &database_name = drop_stmt.object_name;
  
  // 检查数据库是否存在
  if (!db_manager_->DatabaseExists(database_name)) {
    SetError("Database '" + database_name + "' does not exist");
  ...
  // 删除数据库
  if (!db_manager_->DropDatabase(database_name)) {
    return "ERROR: Failed to drop database\n";
  }
  
  return "Query OK, 1 database dropped (" + database_name + ")\n";
}

std::string SqlExecutor::ExecuteDropTable(const sql_parser::DropStatement &drop_stmt) {
  ...
  // 删除表
  if (!db_manager_->DropTable(current_database_, table_name)) {
...
SqlExecutor::ExecuteSelect(const sql_parser::SelectStatement &select_stmt) {
  const std::string &table_name = NormalizeTableName(select_stmt.from_clause.table_name);
  ...
  // 解析SELECT列表
  std::vector<size_t> select_columns = ResolveColumnIndices(select_stmt.select_list, *meta);
  ...
  // 执行查询
  std::vector<Record> results = ExecuteSelectQuery(table_name, *meta, select_stmt);
...
SqlExecutor::ExecuteInsert(const sql_parser::InsertStatement &insert_stmt) {
  ...
  for (const auto &row : insert_stmt.values) {
    ...
    // 约束验证
    if (!ValidateInsertConstraints(table_name, row, GetTableSchema(table_name))) {
...
std::string SqlExecutor::ExecuteUse(const sql_parser::UseStatement &use_stmt) {
  ...
  if (!db_manager_) {
    SetError("Database manager not initialized");
    return "ERROR: Database manager not initialized\n";
  }
  
  const std::string &database_name = use_stmt.database_name;
  ...
  // 切换当前数据库
  bool success = db_manager_->UseDatabase(database_name);
  if (success) {
    current_database_ = database_name;
    return "Database changed to '" + database_name + "'\n";
  } else {
    SetError("Failed to change database to '" + database_name + "'");
    return "ERROR: Failed to change database to '" + database_name + "'\n";
  }
}
...
bool SqlExecutor::TableExists(const std::string &table_name) const {
  if (!db_manager_) {
    // 仅使用内存中的表元数据作为回退
    auto it = table_catalog_.find(NormalizeTableName(table_name));
    return it != table_catalog_.end();
  }
  ...
  return db_manager_->TableExists(current_database_, table_name);
}

const TableMetadata *
SqlExecutor::GetTableMetadata(const std::string &table_name) const {
  // 从DatabaseManager获取表元数据
  if (db_manager_ && !current_database_.empty()) {
    return db_manager_->GetTableMetadata(current_database_, table_name);
  }
...
std::string SqlExecutor::ListTables() {
  ...
  if (db_manager_) {
    ...
    table_names = db_manager_->ListTables(current_database_);
  } else {
    // 仅使用内存中的表元数据作为回退
...
std::string SqlExecutor::NormalizeTableName(const std::string &table_name) const {
  // 简单的表名规范化，去除空白并转为小写
  ...
  // 对于非空数据库和未限定的表名，添加数据库前缀
  if (!current_database_.empty() && normalized.find('.') == std::string::npos) {
    normalized = current_database_ + "." + normalized;
  }
