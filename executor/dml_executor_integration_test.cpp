...
TEST_F(DMLExecutorIntegrationTest, UpdateBasicTest) {
  ...
  std::string insert_sql =
      "INSERT INTO users (id, name, age) VALUES (1, 'Alice', 25);";
  sqlcc::sql_parser::ParserNew insert_parser(insert_sql);
  ...
  sqlcc::sql_parser::ParserNew update_parser(update_sql);
  auto update_stmts = update_parser.parse();
...
TEST_F(DMLExecutorIntegrationTest, DeleteBasicTest) {
  ...
  sqlcc::sql_parser::ParserNew delete_parser(delete_sql);
  auto delete_stmts = delete_parser.parse();
